// AmbienteEnVivo — la pestaña "En vivo" de Ambiente: lectura de los sensores de
// Growcast (temp/HR/CO2/VPD),
...
const WEBHOOK: string = import.meta.env.VITE_GROWCAST_WEBHOOK_URL || 'https://growcast-bridge.grancobud.workers.dev/growcast-live'
...
/** Un nodo del arbol de condiciones de una automatizacion de Growcast. */
interface NodoRegla {
  type?: string
  operator?: string
  value?: { type?: string; hours?: number; minutes?: number }
  children?: NodoRegla[]
}

function parseAuto(a: Auto): { modo: string; icono: LucideIcon; texto: string | null } {
  if (!a) return { modo: 'Manual', icono: Hand, texto: null }
  try {
    const v = JSON.parse(a.valor)
    if (a.tipo === 'schedule') {
      let on: string | null = null, off: string | null = null
      const walk = (n: NodoRegla | undefined) => {
        if (!n) return
        if (n.type === 'comparison' && n.value && n.value.type === 'time') {
          const t = `${String(n.value.hours ?? 0).padStart(2, '0')}:${String(n.value.minutes ?? 0).padStart(2, '0')}`
...
type DefSensor = { key: 'temperatura' | 'humedad' | 'co2' | 'vpd'; rangoKey: 'temp' | 'hr' | 'vpd' | 'co2'; label: string; unidad: string; icono: LucideIcon; color: string; decimales: number }
...
const SALIDAS_DEF: { key: string; label: string; icono: LucideIcon }[] = [
...
export default function AmbienteEnVivo() {
  ...
  return (
    <div className="text-[#d4d4dd]">
      <div className="border-b border-[#1f1f2b]">
        <div className="flex items-center flex-wrap gap-2 sm:gap-x-3 px-3 sm:px-6 py-3">
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#ececf1] flex items-center gap-2">
              <Activity className="w-4 h-4 text-[#bef264]" /> Sensores de Growcast
            </h1>
          ...
          <button onClick={() => setVerFicha(true)} disabled={!data?.sala}
            className="inline-flex items-center justify-center gap-1.5 px-2.5 py-1.5 min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 rounded-lg border border-[#2a2a3a] bg-[#15151d] hover:bg-[#1c1c27] hover:border-[#404d20] transition-colors text-[11px] text-[#a6a6b5] hover:text-[#ececf1] disabled:opacity-40">
          ...
          <button onClick={() => cargar(true)} disabled={cargando} title="Actualizar ahora"
            className="inline-flex items-center justify-center gap-1.5 px-2.5 py-1.5 min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 rounded-lg border border-[#a3e635]/40 bg-[#a3e635]/10 hover:bg-[#a3e635]/20 transition-colors text-[11px] font-medium text-[#d9f99d] disabled:opacity-60">
      ...
      <div className="px-3 sm:px-6 py-4 pb-24 space-y-5">
        {error && !data ? (
          <div className="py-16 text-center">
            ...
            <div className="mt-1 text-[11.5px] text-[#8a8a9c] max-w-sm mx-auto">{error}. Verificá que n8n y la PC estén prendidos.</div>
          ...
          <>
                ...
                return (
                  <div key={s.key} className="rounded-xl bg-[#101016] border border-[#1f1f2b] p-4 flex flex-col">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-[10px] uppercase tracking-[0.12em] text-[#8a8a9c] flex-1">{s.label}</div>
                    ...
                    <div className="mt-2.5 flex items-baseline gap-1.5">
                      {cargando && !v ? <div className="h-8 w-20 bg-[#15151d] rounded animate-pulse" /> : (
                        <>
                          ...
                          <span className="text-[12px] text-[#8a8a9c]">{s.unidad}</span>
                        </>
                      )}
                    </div>
            ...
            <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-[#1f1f2b]">
                ...
                <span className="hidden sm:inline text-[10.5px] text-[#8a8a9c]">cómo se prende y apaga cada cosa</span>
              </div>
                  ...
                  return (
                    <div key={o.key} className="rounded-lg bg-[#15151d] border border-[#1f1f2b] px-3 py-2.5">
                      <div className="flex items-center gap-2.5">
                        <o.icono className="w-4 h-4 flex-shrink-0" style={{ color: on ? '#bef264' : '#8a8a9c' }} />
                        <span className="text-[12.5px] text-[#ececf1] flex-1 truncate">{o.label}</span>
                        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md ${on ? 'text-[#d9f99d] bg-[#a3e635]/10 border border-[#404d20]' : 'text-[#8a8a9c] bg-[#1c1c27] border border-[#2a2a3a]'}`}>
                          {st?.on == null ? '—' : on ? 'ON' : 'OFF'}
                        </span>
                      </div>
                      <div className="mt-1.5 flex items-center gap-1.5 text-[10.5px] text-[#a6a6b5]">
                        <pa.icono className="w-3 h-3 flex-shrink-0 text-[#8a8a9c]" />
            ...
            <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] overflow-hidden">
              <div className="flex items-center flex-wrap gap-2 px-4 py-3 border-b border-[#1f1f2b]">
                ...
                <div className="flex-1" />
                <span className="text-[10px] text-[#8a8a9c]">Etapa:</span>
                <div className="flex gap-1">
                  {(['vegetativo', 'floracion'] as Etapa[]).map(e => (
                    <button key={e} onClick={() => setEtapaManual(e)}
                      className={`px-2 py-1 rounded-md text-[10.5px] font-medium border transition-colors ${etapa === e ? 'border-[#463a66] bg-[#a78bfa]/12 text-[#c4b5fd]' : 'border-[#2a2a3a] bg-[#15151d] text-[#8a8a9c] hover:text-[#a6a6b5]'}`}>
                      {ETAPA_LABEL[e]}
                    </button>
                  ))}
                  {etapaManual && <button onClick={() => setEtapaManual(null)} title="Volver a automático" className="px-2 py-1 rounded-md text-[10.5px] border border-[#2a2a3a] bg-[#15151d] text-[#8a8a9c] hover:text-[#a6a6b5]">Auto</button>}
                </div>
              </div>
              ...
              <div className="px-4 py-2.5 border-t border-[#1f1f2b] text-[10px] text-[#8a8a9c]">
            ...
            <div className="text-[10.5px] text-[#8a8a9c] flex items-center gap-1.5">
  ...
  return (
    <div ref={refDialogo} className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/60 p-0 sm:p-4" onClick={onCerrar}>
      <div className="bg-[#0d0d12] border border-[#1f1f2b] w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-4 -mt-1 text-[10.5px] text-[#8a8a9c]">Datos de la sala en Growcast (solo lectura). Para editarlos, usá la app de Growcast.</div>
        <div className="px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] flex justify-end">
