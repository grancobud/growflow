...
function ComponentesReales() {
  ...
  return (
    <Seccion icon={Boxes} titulo="Componentes del riego" sub="Los insumos que cargaste en la pestaña Insumos">
      {cargando ? (
        <p className="text-[12px] text-[#8a8a9c] py-3">Cargando…</p>
      ) : ordenados.length === 0 ? (
        <p className="text-[12px] text-[#8a8a9c] py-3">
            ...
            return (
              <div key={it.id} className="flex items-center gap-3 bg-[#15151d] border border-[#1f1f2b] rounded-lg px-3 py-2">
                <button title={it.favorito ? 'Quitar de elegidos' : 'Marcar como elegido'} onClick={() => toggleFav(it)} className="flex-shrink-0">
                  <Star className="w-4 h-4" fill={it.favorito ? '#facc15' : 'none'} stroke={it.favorito ? '#facc15' : '#8a8a9c'} />
                </button>
                <div className="min-w-0 flex-1">
                  <p className="text-[12.5px] font-medium text-[#ececf1] truncate">{it.nombre}</p>
                  {desc && <p className="text-[10.5px] text-[#8a8a9c] truncate">{desc}</p>}
                </div>
                <span className="text-[12px] font-mono font-bold text-[#d9f99d] flex-shrink-0">
                  {fmt(it.precio)}{it.unidad ? <span className="text-[10px] text-[#8a8a9c]"> /{it.unidad}</span> : null}
                </span>
              </div>
            )
          })}
      ...
      <p className="text-[11px] text-[#8a8a9c] mt-2">Válvulas 220V = relé directo; la bomba = contactor (ver Hardware DIY). La estrella marca lo que ya elegiste comprar.</p>
    </Seccion>
  )
}
  ...
  return (
    <div className="space-y-4 max-w-[1100px]">
      ...
      <div className={card}>
        ...
        <p className="text-[11px] text-[#8a8a9c] mt-2 border-t border-[#1f1f2b] pt-2">🚧 Estructura inicial — se va completando con el detalle real de tu instalación.</p>
      </div>
      ...
      <Seccion icon={Gauge} titulo="Cómo riega — crop steering (P0/P1/P2)" sub="El método de Growcast: por fases, no un timer">
        ...
        <p className="text-[11px] text-[#8a8a9c] mt-2">Sincronizado con el fotoperiodo (arranca cuando prenden las luces). No hay timer físico: el ciclado está dentro de cada fase.</p>
      </Seccion>
  ...
  - platform: sntp
    on_time:
      - hours: 7                  # cuando prenden las luces...
        ...
        <p className="text-[11px] text-[#8a8a9c] mt-2">Para varias camas: una válvula (switch) por cama, y el script las riega en secuencia o en paralelo según prefieras. Para dryback real, se lee el sensor de humedad en vez de tiempos fijos.</p>
      </Seccion>
