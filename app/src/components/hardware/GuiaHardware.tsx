...
const th = 'text-left font-medium py-1.5 px-2 text-[10px] uppercase tracking-[0.08em] text-[#8a8a9c] border-b border-[#1f1f2b]'
    ...
    <section className={card}>
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <div>
          ...
          {sub && <p className="text-[11px] text-[#8a8a9c] mt-0.5">{sub}</p>}
        </div>
      </div>
      {children}
    </section>
...
export default function GuiaHardware() {
  return (
    <div className="space-y-4 max-w-[1100px]">
      ...
      <Seccion icon={Cpu} titulo="Cómo está hecho el Growcast (ingeniería inversa)" sub="Confirmado bajando y analizando su firmware + API tRPC">
        ...
        <p className="text-[11px] text-[#8a8a9c] mt-3">Firmware descargable: <span className="font-mono text-[#7dd3fc]">firmware.growcast.io?id=&lt;id&gt;</span> · THC = id 2.</p>
      </Seccion>
      ...
      <Seccion icon={Zap} titulo="1 · Cerebro y potencia" sub="El controlador y su alimentación">
        ...
        <p className="text-[10.5px] text-[#8a8a9c] mt-2">Precios relevados jul 2026: Candy-HO (Villa Martelli, candy-ho.com) y MercadoLibre. Los «≈» son rangos de mercado — verificar antes de comprar.</p>
      </Seccion>
      ...
      <Seccion icon={Boxes} titulo="2 · Contactores y protección — dimensionado para TU sala" sub="El relé chico cierra la bobina; el contactor maneja la potencia. 6 contactores">
        ...
        <p className="text-[10.5px] text-[#8a8a9c] mt-2">Precios de referencia (jul 2026): contactor 25A (Chint/BAW/Sica) ≈$25.000-40.000 · contactor 40A ≈$40.000-60.000 · térmica 2x63A ≈$20.000-35.000 · breakers de grupo (C10/C16/C32) ≈$8.000-15.000 c/u. Todo en ML / casas de electricidad; el matriculado suele conseguirlos mejor.</p>
      ...
      <Seccion icon={Thermometer} titulo="3 · Sensor principal — el «THC» de Growcast" sub="Un solo chip = CO₂ + Temperatura + Humedad + VPD">
        ...
        <p className="text-[11px] text-[#8a8a9c]">⚠️ Algunos módulos rotulan «41» pero traen SCD40. Al conectarlo, ESPHome reporta el chip y su rango — verificar ahí.</p>
      </Seccion>
  ...
  (en el tablero)                                            ──→  [nodo sensor 3] ...`}</pre>
      ...
      <Seccion icon={Wrench} titulo="5 · Control avanzado (features tipo TrolMaster)" sub="Lo que Growcast NO tiene">
        ...
        />
        <p className="text-[11px] text-[#8a8a9c] mt-2">El override manual va en serie con la salida (o con un toggle de 3 posiciones ON–AUTO–OFF entre el relé/contactor y la carga). Es la seguridad de poder operar la sala aunque falle la electrónica.</p>
      </Seccion>
      ...
      <Seccion icon={Droplets} titulo="5b · Riego de las 3 camas (fertirriego)" sub="Tu esquema: UNA electroválvula para las 3 camas en paralelo + bomba. Válvula por cama = upgrade opcional">
        ...
        <p className="text-[11px] text-[#8a8a9c] mt-2">Growcast soporta sensores de sustrato METER (TEROS 12 = humedad + temp + EC del sustrato) — la referencia si querés riego por humedad de verdad, no solo por horario.</p>
        <div className="mt-3 rounded-lg bg-[#a3e635]/[0.06] border border-[#404d20] p-3">
          ...
          <p className="text-[11px] text-[#8a8a9c] mt-2">No hay timer físico: el ciclado está dentro de cada fase (On/Off/repeticiones), corriendo on-device. En ESPHome se replica con <span className="font-mono">script</span> + <span className="font-mono">interval</span> + la lectura del sensor de humedad.</p>
        </div>
      </Seccion>
...
climate: ...`}</pre>
      ...
      <Seccion icon={Monitor} titulo="8 · «Cerebro visual» (opcional)" sub="Para el dashboard tipo app de Growcast, offline">
        ...
        />
        <p className="text-[11px] text-[#8a8a9c] mt-2">Recomendación: arrancá sin Pi (ESP32 + growflow). Sumá Home Assistant en una Pi si después querés el panel local completo.</p>
      </Seccion>
      ...
      <Seccion icon={Boxes} titulo="Diagramas para armarlo (visual)" sub="La idea en dibujos: cómo prende una carga y cómo se ordena el tablero por dentro">
        ...
        <p className="text-[11px] text-[#8a8a9c] mb-4">El relé chico (que dispara el ESP32) <b>solo cierra la bobina</b> del contactor. La corriente pesada (AC, luces, bomba) pasa por el <b>contactor</b>, nunca por el relé. Así el ESP32 nunca ve los 220V.</p>
        ...
        <p className="text-[11px] text-[#8a8a9c] mb-1">De arriba a abajo: <b className="text-[#7dd3fc]">breakers</b> → <b className="text-[#a3e635]">lógica (ESP32 + relés)</b> y <b className="text-[#a78bfa]">sensores</b> → <b className="text-[#f0a35e]">contactores</b> → <b>bornera + tierra</b>. Cada relé de la placa verde cierra la bobina del contactor naranja de abajo.</p>
      </Seccion>
      ...
      <Seccion icon={Cable} titulo="Esquema de conexión (pinout + tablero)" sub="Pines elegidos evitando los conflictivos del ESP32 (flash 6-11, strapping 0/2/12/15, solo-entrada 34-39)">
        ...
        />
        <p className="text-[11px] text-[#8a8a9c] mt-2">Todos los pines de relé son salidas seguras (no strapping, no solo-entrada). La placa de relés es activa en LOW → en ESPHome van con <span className="font-mono">inverted: true</span> y <span className="font-mono">restore_mode: ALWAYS_OFF</span>.</p>
...
Snubber/varistor en paralelo a las cargas inductivas (AC, bomba)`}</pre>
        <p className="text-[11px] text-[#8a8a9c] mt-2">El relé chico solo cierra la <b>bobina</b> del contactor; la potencia (29A del AC) pasa por el contactor. Multímetro ANTES de energizar: continuidad, sin cortos, tierra OK.</p>
      </Seccion>
  ...
  - interval: 30min
    then:
      ...
      - switch.turn_off: rele_vent`}</pre>
        <p className="text-[11px] text-[#8a8a9c] mt-2">A mano lo prendés desde el celu/growflow/Home Assistant (el botón que crea el <span className="font-mono">switch</span>). Los otros 3 son automáticos.</p>
  ...
  - platform: sntp
    on_time:
      - hours: 7                  # cuando prenden las luces...
        ...
        <p className="text-[11px] text-[#8a8a9c] mt-2">Cambiás los números (2h, 30s, 6 veces…) y tenés tu crop steering. Para riego por humedad real (dryback), en vez de tiempos fijos leés el sensor de sustrato: «si bajó del 60% → regá».</p>
      </Seccion>

      <p className="text-[10.5px] text-[#8a8a9c] px-1 pb-4">
