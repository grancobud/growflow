  ...
  return (
    <div className="space-y-3">
      ...
      {items.length === 0 && !nuevoOpen && (
        <p className="text-[12px] text-[#8a8a9c] py-6 text-center">Todavía no cargaste insumos de riego. Tocá «Nuevo insumo».</p>
      )}
          ...
          <div className="w-full flex items-center gap-2.5">
            ...
            <button onClick={() => toggle(it.id)} className="flex-1 flex items-center gap-3 text-left min-w-0">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10.5px] text-[#8a8a9c]">{[it.marca, it.modelo, it.specs].filter(Boolean).join(' · ') || 'Sin datos'}</p>
              </div>
              <span className="text-[13px] font-mono font-bold text-[#d9f99d]">{fmt(it.precio)}{it.unidad ? <span className="text-[10px] text-[#8a8a9c]"> /{it.unidad}</span> : null}</span>
              <ChevronDown className={`w-4 h-4 text-[#8a8a9c] transition-transform ${abierto === it.id ? 'rotate-180' : ''}`} />
            </button>
          </div>
                  ...
                  editOferta === o.id ? (
                    ...
                    <div key={o.id} className="flex items-center gap-2 bg-[#15151d] border border-[#1f1f2b] rounded-lg p-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10.5px] text-[#8a8a9c] truncate">{[o.presentacion, o.nota].filter(Boolean).join(' · ')}</p>
                      </div>
                      ...
                      <button title="Usar este precio" className={btnIcono} onClick={async () => { await instalacionesService.elegirOferta(o.id, it.id, o.precio ?? null, o.proveedor_id ?? null); await cargar(); }}>
                        <Star className="w-4 h-4" fill={o.elegido ? '#facc15' : 'none'} stroke={o.elegido ? '#facc15' : '#8a8a9c'} />
                      </button>
                      <button title="Editar proveedor" aria-label="Editar proveedor" className={btnIcono} onClick={() => setEditOferta(o.id)}>
                        <Pencil className="w-3.5 h-3.5 text-[#8a8a9c]" />
                      </button>
  ...
  return (
    <div className={card}>
      ...
      <div className="flex items-center gap-2 mt-2">
        ...
        {onCancelar && <button onClick={onCancelar} className="text-[11px] text-[#8a8a9c] px-2 py-2 min-h-[44px] sm:min-h-0 hover:text-[#a6a6b5]">Cancelar</button>}
      </div>
    </div>
  )
}
