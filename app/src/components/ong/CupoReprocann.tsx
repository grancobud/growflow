    ...
    <div className={card}>
      <button onClick={onToggle} className="w-full flex items-center gap-2 text-left min-h-[44px] sm:min-h-0">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[10.5px] mt-0.5" style={{ color: sinPermiso ? '#ff8a7a' : '#8a8a9c' }}>
            {p.reprocann
        ...
        <div className="text-right flex-shrink-0">
          ...
          <p className="text-[9.5px] uppercase tracking-[0.1em] text-[#8a8a9c]">en floración</p>
        </div>
      ...
      {abierto && (
        <div className="mt-3 pt-3 border-t border-[#1f1f2b] space-y-1.5">
          {plantas.length === 0
            ? <p className="text-[11.5px] text-[#8a8a9c]">Sin plantas imputadas. Tiene {p.libres} lugares libres.</p>
            : plantas.map(pl => (
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg bg-[#15151d] border border-[#1f1f2b] px-2 sm:px-3 py-2 min-h-[44px]">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[10.5px] text-[#8a8a9c] truncate">{[pl.genetica, pl.fase].filter(Boolean).join(' · ')}</p>
      </div>
      {asignando === pl.id
