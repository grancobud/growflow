...
//
// Lo pidió Panacea el 02/09/2026, y con el orden puesto: «llega el encargado a
...
export interface EstadoAmbiente {
  ...
  detalle: string
  turno: string
  /** El titulo del paso cuando falta la lectura (ver `tituloFaltaLectura`). */
  tituloFalta: string
}
    ...
    <section aria-labelledby="apertura-titulo"
      ...
      {ambiente && ambiente.falta !== null && (
        <Paso n={++n} estado={ambiente.falta ? 'pendiente' : 'hecho'} icono={Thermometer} a="/ambiente"
          titulo={ambiente.falta ? ambiente.tituloFalta : 'Valores de la sala'}
