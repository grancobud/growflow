    ...
    <div className="-mt-1 -mx-1">
      ...
      {abierto && (
        <div data-panel-tabs role="group" aria-label="Más herramientas"
          ...
          {GRUPOS_TABS.map(g => (
            <div key={g.grupo}>
              <div className="px-3 pt-3 pb-1 text-[11px] uppercase tracking-[0.12em] text-[#8a8a9c] font-medium">
                {g.grupo}
              </div>
...
function colorDelta(logrado: number, obj: number) {
  if (obj <= 0) return { c: '#8a8a9c', bg: 'transparent' }
  const r = logrado / obj
    ...
    <>
      <span className="relative inline-flex group align-middle ml-1">
        <button type="button" onClick={() => setAbierto(v => !v)} aria-expanded={abierto}
          ...
          className="p-2.5 -m-2.5 min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 sm:p-0 sm:m-0 flex items-center justify-center cursor-help group">
...
const GRUPOS_PRESET = [
  { id: 'base', etiqueta: 'Base', color: 'text-[#8a8a9c]', test: (id: string) => !/^(ryano_|co2_)/.test(id) },
  { id: 'ryano', etiqueta: 'Ryanodine', color: 'text-[#c4b5fd]', test: (id: string) => id.startsWith('ryano_') },
    ...
    <div className="space-y-4">
      ...
      <div className={`${card} space-y-3`}>
        <div className="grid gap-3 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
          <div>
            <div className="flex items-baseline gap-2 mb-2">
              <p className="text-[11.5px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium">
                1 · Perfil objetivo <span className="text-[#8a8a9c] normal-case tracking-normal">— qué quiero lograr</span>
              </p>
              {presetActivo && (
          ...
          <div>
            <p className="text-[11.5px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-2">
              2 · Kit de sales <span className="text-[#8a8a9c] normal-case tracking-normal">— con qué lo hago</span>
            </p>
            <div className="flex flex-col gap-1.5">
              {KITS_SALES.map(kit => (
                <button key={kit.id} onClick={() => setActivas(new Set(kit.sales))} title={kit.desc}
                  ...
                  <span className="block text-[11.5px] text-[#8a8a9c] leading-snug mt-0.5 line-clamp-2">{kit.desc}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="border-t border-[#1f1f2b] pt-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-[11.5px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium">Mis perfiles</p>
            ...
            <span className="ml-auto text-[11.5px] text-[#8a8a9c] tabular-nums">{guardados.length}</span>
          </div>
              ...
              {guardados.map((g: PerfilGuardado) => (
                <div key={g.id}
                  ...
                  <button onClick={() => cargarPerfil(g)} title="Cargar este perfil"
                    className="flex items-center gap-1.5 text-[13px] text-[#d4d4dd] hover:text-[#d9f99d] min-h-[44px] max-w-[190px] truncate">
                    <FolderOpen className="w-3.5 h-3.5 flex-shrink-0 text-[#8a8a9c] group-hover:text-[#a3e635]" strokeWidth={1.8} />
                    <span className="truncate">{g.nombre}</span>
                  </button>
                  <button onClick={() => borrarPerfil(g)} title={`Borrar "${g.nombre}"`} aria-label={`Borrar ${g.nombre}`}
                    className="p-1.5 rounded text-[#8a8a9c] hover:text-[#ff8a7a] hover:bg-[#ff8a7a]/10 transition-colors flex-shrink-0">
      ...
      <div className="grid lg:grid-cols-2 gap-4 items-start">
        ...
        <div className={card}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="ml-auto text-[11.5px] text-[#8a8a9c]">N total: {nLog.toFixed(0)}</span>
          </div>
          ...
          <details className="mt-3 group" open>
            <summary className="text-[11.5px] uppercase tracking-[0.12em] text-[#8a8a9c] cursor-pointer flex items-center gap-1">
          ...
          <div className="flex items-center gap-x-2 gap-y-2 mb-3 flex-wrap">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              <label className="flex items-center gap-1 text-[12px] text-[#a6a6b5]">
                <span className="text-[#8a8a9c]">prepará</span>
                <NumField value={litros} onChange={setLitros} min={0.1}
                  className="w-14 bg-[#15151d] border border-[#1f1f2b] rounded px-1.5 py-1.5 sm:py-0.5 min-h-[44px] sm:min-h-0 text-[16px] sm:text-[13.5px] text-[#ececf1] font-mono tabular-nums text-center focus:border-[#404d20] outline-none" />
                <span>L</span>
              </label>
              ...
              <div className="flex rounded-md overflow-hidden border border-[#1f1f2b]">
                {(['polvo', 'liquido'] as const).map(m => (
                  <button key={m} onClick={() => setModoPrep(m)}
                    className={`px-2.5 py-2 sm:py-0.5 min-h-[44px] sm:min-h-0 text-[12px] font-medium transition-colors ${modoPrep === m ? 'bg-[#a3e635]/15 text-[#d9f99d]' : 'bg-[#15151d] text-[#8f8f9f] hover:text-[#d4d4dd]'}`}>
            ...
            <button onClick={() => imprimirReceta({ nombre: p.nombreNuevo, perfil, res, porBidon, ec, litros, modoPrep, resolucion })}
              ...
              className="text-[12px] flex items-center gap-1 flex-shrink-0 px-2 py-2 sm:py-1 min-h-[44px] sm:min-h-0 rounded-md bg-[#15151d] border border-[#1f1f2b] text-[#a6a6b5] hover:text-[#d9f99d] hover:border-[#404d20] transition-colors disabled:opacity-40">
              <Printer className="w-3.5 h-3.5" strokeWidth={1.8} /> Imprimir
            </button>
          </div>
          {porBidon.length === 0 ? (
            <p className="text-[13.5px] text-[#8a8a9c] py-6 text-center">Sin sales que cubran el objetivo. Activá más en "Sustancias".</p>
          ) : (
            <div className="space-y-3">
              {/* Qué de esta receta se puede comprar hoy y qué no. */}
              <div className="flex items-center gap-3 flex-wrap text-[12.5px] text-[#8a8a9c] -mt-1">
                    ...
                    {g.items.map((d: ResultadoSal) => {
                      ...
                      <div key={d.sal.id} className="flex items-center gap-2 bg-[#15151d] border border-[#1f1f2b] rounded-md px-2.5 py-1.5">
                        ...
                        <span className="text-[13.5px] font-mono tabular-nums font-bold text-[#ececf1]">
                          {gv >= 0.01 ? gv.toFixed(2) : gv >= 0.001 ? gv.toFixed(4) : gv.toFixed(6)} <span className="text-[#8a8a9c] font-normal">{litros === 1 ? 'g/L' : 'g'}</span>
                        </span>
                      </div>
                      )
                    })}
        ...
        return (
          <div className="rounded-xl bg-[#101016] border border-[#facc15]/25 overflow-hidden">
            ...
            <div className="p-3 space-y-2">
              ...
              {stocks.map(s => (
                <div key={s.salId} className="rounded-lg bg-[#15151d] border border-[#1f1f2b] px-3 py-2">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-[11.5px] text-[#8a8a9c] font-mono">pedía {s.gramosPorL < 0.001 ? s.gramosPorL.toFixed(6) : s.gramosPorL.toFixed(4)} g/L · impesable</span>
                  </div>
              ...
              <p className="text-[11.5px] text-[#8a8a9c]">El stock te dura muchos lotes. Guardalo rotulado en la heladera. Ajustá la precisión de balanza en "Soluciones madre".</p>
            </div>
          </div>
        )
      })()}
      ...
      <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] overflow-hidden">
        <div className="px-4 py-3 border-b border-[#1f1f2b] flex items-center gap-2">
          ...
          <button onClick={() => setEditarRangos(v => !v)} className="ml-auto text-[11.5px] px-2 py-1.5 sm:py-0.5 min-h-[44px] sm:min-h-0 rounded border border-[#1f1f2b] text-[#8f8f9f] hover:text-[#d9f99d] hover:border-[#404d20] transition-colors">
          ...
          <table className="w-full text-[12.5px]">
            <thead><tr className="border-b border-[#1f1f2b]">
              {['Elemento', 'Objetivo', 'Logrado', 'Rango min–max', 'Estado'].map(h => (
                <th key={h} className="px-3 py-2 text-left text-[11.5px] uppercase tracking-[0.1em] text-[#8a8a9c] font-medium">{h}</th>
              ))}
            </tr></thead>
            <tbody>
...
function SugerenciaSalesActivas({ salesTodas, activas, setActivas }: { salesTodas: Sal[]; activas: Set<string>; setActivas: SetSet }) {
  return (
    <p className="text-[11.5px] text-[#8a8a9c] px-1">
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          ...
          <span className="ml-auto text-[11.5px] text-[#8a8a9c]">{visibles.length} visibles</span>
        ...
        {(() => {
          ...
          return ordenadas.map(([orden, g]) => (
            <div key={orden} className="mt-3">
              <p className="text-[11.5px] uppercase tracking-[0.14em] text-[#8a8a9c] font-semibold mb-1.5 px-0.5">{g.label} <span className="text-[#8a8a9c]">· {g.items.length}</span></p>
                ...
                <div className="flex items-start gap-2 px-2.5 py-1.5">
                  ...
                  <button onClick={() => setAbierta(open ? null : s.id)} className="min-w-0 flex-1 text-left">
                    <span className={`block text-[13px] font-medium ${on ? 'text-[#d9f99d]' : 'text-[#a6a6b5]'}`}>
                      {s.nombre} {s.formula && <span className="text-[#8a8a9c] font-normal">{s.formula}</span>}
                    ...
                    <span className="block text-[11.5px] text-[#8a8a9c] mt-0.5 line-clamp-2">{s.descripcion ?? s.nota ?? 'Sin descripción.'}</span>
                    {(() => {
                      const u = usos[s.id]
                  ...
                  <ChevronDown className={`w-4 h-4 mt-0.5 text-[#8a8a9c] flex-shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} />
                </div>
...
function FichaSal({ sal, onSaved, onDelete }: { sal: Sal; onSaved: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="border-t border-[#1f1f2b] px-3 py-3 space-y-3">
      <div>
        <p className="text-[11.5px] uppercase tracking-[0.12em] text-[#8a8a9c] mb-1">Qué es / para qué sirve</p>
        <p className="text-[12.5px] text-[#c4c4d0]">{sal.descripcion ?? 'Sin descripción.'}</p>
      </div>
      <div>
        <p className="text-[11.5px] uppercase tracking-[0.12em] text-[#8a8a9c] mb-1">Compatibilidad de mezcla</p>
        <p className="text-[12.5px] text-[#c4c4d0]">{compatibilidad(sal)}</p>
      </div>
      <div>
        <p className="text-[11.5px] uppercase tracking-[0.12em] text-[#8a8a9c] mb-1">Composición</p>
        <p className="text-[12.5px] text-[#a6a6b5] font-mono">{comp || '—'} <span className="text-[#8a8a9c]">· bidón {sal.bidon}{sal.liquido ? ' · líquido' : ''}</span></p>
      </div>
      {sal.docs && sal.docs.length > 0 && (
        <div>
          <p className="text-[11.5px] uppercase tracking-[0.12em] text-[#8a8a9c] mb-1.5">Fichas técnicas / PDF</p>
          <div className="space-y-1.5">
            {sal.docs.map(d => (
      ...
      <div>
        <p className="text-[11.5px] uppercase tracking-[0.12em] text-[#8a8a9c] mb-1.5">Mi inventario (editable)</p>
      ...
      <div className="flex gap-2 justify-end">
        {sal.custom && (
          <button onClick={async () => { try { await sustanciasService.eliminar(sal.id); await onDelete(); toast.success('Sustancia eliminada') } catch (e) { toast.error(String(e)) } }}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[13.5px] text-[#8a8a9c] hover:text-[#ff8a7a] hover:bg-[#ff8a7a]/10">
...
function NuevaSustancia({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="rounded-lg bg-[#15151d] border border-[#404d20] p-3 mb-2 space-y-3">
      ...
      {comercial && <p className="text-[11.5px] text-[#8a8a9c]">Cargá los números tal cual la etiqueta (N, P₂O₅, K₂O, CaO, MgO, SO₃). Se convierten a elemental solos. Ej: tu Ryanodine Maikro/Calcis.</p>}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
...
function GrupoAgua({ titulo, items, agua, set }: { titulo: string; items: Elem[]; agua: Perfil; set: (k: ElementKey, v: number) => void }) {
  return (
    <div>
      <p className="text-[11.5px] uppercase tracking-[0.12em] text-[#8a8a9c] font-medium mb-2">{titulo}</p>
  ...
  return (
    <div className={`${card} space-y-4`}>
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto text-[11.5px] text-[#8a8a9c]">RO = 0 · se descuenta del objetivo</span>
      </div>
      <p className="text-[12.5px] text-[#8a8a9c]">Cargá lo que ya trae tu agua (remineralizador, canilla, pozo). El cálculo lo resta para no pasarte.</p>
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-4">
          <label className="text-[12.5px] text-[#a6a6b5]">Factor de concentración
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <span className="text-[#8a8a9c]">x</span>
            </div>
          </label>
          ...
          <div className="text-[12.5px] text-[#8a8a9c] self-end">
      ...
      {botellasGuardadas.length === 0 ? (
        <p className="text-[13.5px] text-[#8a8a9c] py-6 text-center">Todavía no tenés clones guardados. Cloná un producto y guardalo con un nombre en la pestaña Calculadora.</p>
      ) : (
        <div className="pt-2">
    ...
    <div className={card}>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-[12.5px] border-collapse min-w-[640px]">
          <thead>
            <tr className="text-[10.5px] uppercase tracking-[0.1em] text-[#8a8a9c] text-left">
            ...
            {filas.map(({ f, cmp, diyPorLitro, ahorro }) => (
              <tr key={f.id} className="border-t border-[#1f1f2b]">
                <td className="py-2 pr-2">
                  ...
                  {f.envase_cant && (
                    <span className="text-[#8a8a9c]"> · {fmtNum(f.envase_cant)} {f.envase_unidad}
                ...
                <td className="py-2 px-2 text-right font-mono tabular-nums font-bold"
                  style={{ color: ahorro == null ? '#8a8a9c' : ahorro > 0 ? '#bef264' : '#ff8a7a' }}>
      ...
      <p className="text-[11.5px] text-[#8a8a9c] mt-3">
    ...
    <div className="w-full sm:w-[230px] rounded-lg bg-[#15151d] border border-[#1f1f2b] p-3">
      {ahorro != null && (
        <div className="text-center pb-2.5 mb-2.5 border-b border-[#1f1f2b]">
          ...
          <div className="text-[10px] uppercase tracking-[0.12em] text-[#8a8a9c] mt-1">
      ...
      <dl className="space-y-1.5">
        {filas.map(f => (
          <div key={f.l} className="flex items-baseline justify-between gap-2">
            <dt className="text-[11px] text-[#8a8a9c]">{f.l}</dt>
            <dd className="text-[12.5px] font-mono tabular-nums" style={{ color: f.c }}>{f.v}</dd>
          </div>
        ))}
      </dl>
      {ficha?.precio_envase != null && ficha.envase_cant && (
        <p className="text-[10.5px] text-[#8a8a9c] mt-2 pt-2 border-t border-[#1f1f2b] leading-snug">
...
function ColMicros({ titulo, sub, dosis, litros, resolucion, acento, elegida, onElegir, conProveedor }: { titulo: string; sub: string; dosis: ResultadoSal[]; litros: number; resolucion: number; acento: string; elegida?: boolean; onElegir?: () => void; conProveedor?: Set<string> }) {
  ...
  return (
    <div className={`flex-1 min-w-0 rounded-lg bg-[#101016] border p-3 ${elegida ? 'border-[#facc15]/60' : 'border-[#1f1f2b]'}`}>
      <div className="flex items-center gap-2 mb-0.5">
        ...
        {onElegir && (
          <button onClick={onElegir} title="Usar esta forma en la receta"
            className={`flex items-center gap-1 text-[11.5px] px-2 py-1.5 sm:py-0.5 min-h-[44px] sm:min-h-0 rounded border ${elegida ? 'bg-[#facc15]/15 border-[#facc15]/50 text-[#facc15]' : 'border-[#2a2a3a] text-[#8f8f9f] hover:text-[#facc15] hover:border-[#facc15]/40'}`}>
            <Star className="w-3 h-3" fill={elegida ? '#facc15' : 'none'} strokeWidth={1.8} /> {elegida ? 'Elegida' : 'Usar esta'}
          </button>
        )}
      </div>
      <div className="flex items-baseline gap-2 mb-2">
        <p className="text-[11.5px] text-[#8a8a9c] flex-1 min-w-0">{sub}</p>
        {conProveedor && (
      ...
      {dosis.length === 0 ? (
        <p className="text-[12px] text-[#8a8a9c] py-2">Sin micros en este perfil.</p>
      ) : (
        <div className="space-y-1">
          {dosis.map(d => {
            ...
            <div key={d.sal.id} className="flex items-center gap-2 bg-[#15151d] border border-[#1f1f2b] rounded-md px-2.5 py-1.5">
              ...
              <span className="text-[13px] font-mono tabular-nums font-bold text-[#ececf1]">{fmtG(d.gramosPorL, litros, resolucion)} <span className="text-[#8a8a9c] font-normal">{litros === 1 ? 'g/L' : 'g'}</span></span>
            </div>
            )
          })}
        </div>
...
function PanelMicros2({ perfil, salesTodas, litros, resolucion, activas, setActivas, conProveedor }: { perfil: Perfil; salesTodas: Sal[]; litros: number; resolucion: number; activas: Set<string>; setActivas: SetSet; conProveedor?: Set<string> }) {
  ...
  return (
    <div className={card}>
      ...
      <p className="text-[12.5px] text-[#8a8a9c] mb-3 leading-relaxed">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-[12.5px] border-collapse">
          <thead>
            <tr className="text-[#8a8a9c] text-[11.5px] uppercase tracking-[0.1em]">
            ...
            {micros.map(([k, nombre]) => {
              ...
              return (
                <tr key={k} className="border-t border-[#1f1f2b]">
                  <td className="py-1 text-[#d4d4dd]">{nombre} <span className="text-[#8a8a9c]">({k})</span></td>
...
function HoclDilucionCalc() {
  ...
  return (
    <div className="mt-3 rounded-lg bg-[#101016] border border-[#2f5a72]/60 p-3">
      ...
      <p className="text-[12px] text-[#8a8a9c] mb-3">Con el generador de HOCl en polvo (ej. Binal BioMax) armás el stock y lo dosificás en el riego. Misma dosis que el Cleanse.</p>
      <div className="flex flex-wrap items-end gap-4 mb-3">
        ...
        <label className="text-[12.5px] text-[#a6a6b5]">Dosis en el riego
          <div className="flex items-center gap-1 mt-1">
            ...
            <span className="text-[#8a8a9c]">mL/L</span>
          </div>
        </label>
      </div>
      <div className="grid sm:grid-cols-3 gap-2 text-center">
        <div className="rounded-md bg-[#15151d] border border-[#1f1f2b] px-2 py-2">
          <p className="text-[11.5px] text-[#8a8a9c]">1 · Pesá polvo HOCl</p>
          <p className="text-[16px] font-mono font-bold text-[#bef264]">{num(polvo)} g</p>
        </div>
        <div className="rounded-md bg-[#15151d] border border-[#1f1f2b] px-2 py-2">
          <p className="text-[11.5px] text-[#8a8a9c]">2 · Disolvé hasta</p>
          <p className="text-[16px] font-mono font-bold text-[#7dd3fc]">{num(litrosStock)} L</p>
        </div>
        <div className="rounded-md bg-[#15151d] border border-[#1f1f2b] px-2 py-2">
          <p className="text-[11.5px] text-[#8a8a9c]">3 · Rinde riego</p>
          <p className="text-[16px] font-mono font-bold text-[#a78bfa]">{num(rindeL)} L</p>
        </div>
      </div>
      <p className="text-[11.5px] text-[#8a8a9c] mt-2">Base: 0,5 g/L de polvo en el stock (= 2,5 g en 5 L) y {num(dosisMlL)} mL de stock por litro de riego. Guardá el stock al reparo de la luz.</p>
    </div>
  )
}
  ...
  return (
    <div className="flex items-center gap-2 bg-[#15151d] border border-[#1f1f2b] rounded-md px-3 py-2">
      <div className="flex-1 min-w-0">
        ...
        {nota && <p className="text-[11.5px] text-[#8a8a9c] truncate">{nota}</p>}
      </div>
      {pct && <span className="text-[11.5px] text-[#8a8a9c] tabular-nums">{pct}</span>}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={card}>
        <div className="flex flex-wrap items-end gap-4 mb-3">
          <label className="text-[12.5px] text-[#a6a6b5]">Gel a preparar
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <span className="text-[#8a8a9c]">g</span>
            </div>
          </label>
          ...
          <div className="text-[12.5px] text-[#8a8a9c] self-end">Rinde ~<b className="text-[#bef264]">{clonesAprox.toLocaleString('es-AR')}</b> clones</div>
        </div>
        ...
        <p className="text-[11.5px] text-[#8a8a9c] mt-2">Colorante violeta de cristal (1 gota) y trazas de antifúngico son solo estéticos/marketing — opcionales.</p>
      </div>
      ...
      <div className={card}>
        ...
        <p className="text-[11.5px] text-[#8a8a9c] mt-3">Fuentes: MSDS Clonex (HEC 1,2 % / IBA 0,3 %), foros Rollitup y THCFarmer, literatura de propagación (rango óptimo 1.000–3.000 ppm IBA).</p>
      </div>
    </div>
  )
}
    ...
    <div className="space-y-4">
      ...
      <div className={card}>
        <div className="flex flex-wrap items-end gap-4 mb-3">
          ...
          <label className="text-[12.5px] text-[#a6a6b5]">Concentración objetivo
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <span className="text-[#8a8a9c]">ppm</span>
            </div>
          </label>
        ...
        <div className="rounded-lg bg-[#101016] border border-[#7dd3fc]/20 p-3 mb-2">
          ...
          <p className="text-[22px] font-mono font-bold text-[#7dd3fc] mt-0.5">{num(gramos)} g <span className="text-[14px] text-[#8a8a9c]">de Binal BioMax</span></p>
          <p className="text-[12px] text-[#8a8a9c] mt-0.5">= {num(gPorL)} g/L → ~{num(ppm)} ppm de HOCl</p>
        </div>
        <div className="grid sm:grid-cols-2 gap-2">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={card}>
        ...
        <p className="text-[11.5px] text-[#8a8a9c] mt-3">Dosis de estudios en Cannabis sativa/hemp (quitosano 50–250 mg/L, salicílico ~200 µM) + recetas de foros (kelp/fúlvico/vit B). Insumos cargados en Sustancias.</p>
      </div>
    </div>
  )
}
      ...
      <div className={card}>
        ...
        <div className="overflow-x-auto mt-3 -mx-3 px-3">
          <table className="w-full min-w-[620px] text-[12.5px] border-collapse">
            <thead>
              <tr className="text-[#8a8a9c] text-[11.5px] uppercase tracking-[0.1em] text-left">
            ...
            <tbody>
              {BIOESTIM_INSUMOS.map((b, i) => (
                <tr key={i} className="border-t border-[#1f1f2b] align-top">
                  <td className="py-2 pr-2 text-[#d9f99d] font-medium">{b.nombre}<p className="text-[11.5px] text-[#8a8a9c] font-normal">{b.nota}</p></td>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`${card} border-[#facc15]/25`}>
        ...
        <p className="text-[11.5px] text-[#8a8a9c] mt-3">Dosis basadas en estudios de biostimulación en Cannabis sativa (sensibilidad a aa 0,1 mM; triptófano→auxina) + literatura de vitaminas B/C en plantas. Insumos en Sustancias.</p>
      </div>
    </div>
  )
}
...
function MatrizCompatibilidad({ sales }: { sales: Sal[] }) {
  ...
  return (
    <div className={card}>
      ...
      <p className="text-[12px] text-[#8a8a9c] mb-3">Cruzá dos sales para ver si conviven en la misma botella concentrada. Pasá el mouse por cada celda para el motivo.</p>
      ...
      <p className="text-[11.5px] text-[#8a8a9c] mt-1">Basada en la tabla profesional de compatibilidad de fertirriego (IPNI) + las reglas del concentrado (Ca no va con sulfatos/fosfatos/carbonatos en stock).</p>
    </div>
  )
}
...
function BotellaSVG({ color, letra, volumenL }: { color: string; letra: string; volumenL: number }) {
  return (
    <div className="flex flex-col items-center flex-shrink-0">
      ...
      <span className="text-[11.5px] text-[#8a8a9c] mt-0.5 tabular-nums">{volumenL} L</span>
    </div>
  )
}
        ...
        <div key={g.bidon} className="rounded-lg bg-[#101016] border border-[#1f1f2b] p-3 w-full sm:w-[320px]">
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1 min-w-0 space-y-1">
              ...
              <p className="text-[11.5px] text-[#8a8a9c] pt-1">+ agua hasta {g.volumenL} L</p>
            </div>
          </div>
...
function ProveedoresTab({ salesTodas, recargarInventario, recargarProveedores }: { salesTodas: Sal[]; recargarInventario: () => void; recargarProveedores: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {provs.length === 0 ? (
        <p className="text-[13.5px] text-[#8a8a9c] py-6 text-center">Todavía no cargaste proveedores. Agregá el primero arriba.</p>
      ) : provsFiltrados.length === 0 ? (
        <p className="text-[13.5px] text-[#8a8a9c] py-6 text-center">No hay proveedores para esa sal.</p>
      ) : (
        <div className="space-y-3">
          {[...porSal.entries()].map(([salId, lista]) => (
            <div key={salId} className={`${card} overflow-hidden`}>
              <p className="text-[13.5px] font-display font-semibold text-[#d9f99d] mb-2 break-words">{salNombre(salId)} <span className="text-[11.5px] text-[#8a8a9c]">· {lista.length} proveedor{lista.length > 1 ? 'es' : ''}</span></p>
              <div className="grid sm:grid-cols-2 gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={e => { e.stopPropagation(); toggleElegido(p) }} title={p.elegido ? 'Precio de referencia (clic para quitar)' : 'Usar este precio para calcular el costo'}
                          className="ml-auto p-0.5 rounded hover:scale-110 transition-transform" style={{ color: p.elegido ? '#facc15' : '#8a8a9c' }}>
                          <Star className="w-4 h-4" strokeWidth={1.8} fill={p.elegido ? '#facc15' : 'none'} />
                        </button>
                        <button onClick={e => { e.stopPropagation(); borrar(p) }} title="Borrar proveedor" className="p-0.5 rounded text-[#8a8a9c] hover:text-[#ff8a7a]"><Trash2 className="w-3.5 h-3.5" strokeWidth={1.8} /></button>
                      </div>
                      ...
                      <div className="text-[12px] text-[#a6a6b5] mt-0.5 space-y-0.5 min-w-0">
                        {p.precio != null && <div className="font-mono truncate"><span className="text-[#d4d4dd]">${p.precio}</span> <span className="text-[#8a8a9c]">/ {p.unidad}</span>{precioPorKg(p.precio, p.unidad) != null && <span className="text-[#a3e635]"> · ${precioPorKg(p.precio, p.unidad)}/kg</span>}{p.presentacion ? <span className="text-[#8a8a9c]"> · {p.presentacion}</span> : null}</div>}
                        ...
                        {p.nota && <div className="text-[11.5px] text-[#8a8a9c] line-clamp-1">{p.nota}</div>}
                      </div>
                      <p className="text-[10.5px] text-[#8a8a9c] mt-1">Tocá para ver / editar ▸</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <p className="text-[11.5px] text-[#8a8a9c] px-1">Podés cargar varios proveedores por sal. Tocá una ficha para ver el detalle y editarla. Se ordenan por calidad (Alta primero); prioriza calidad, no precio.</p>
          ...
          <div className="bg-[#101016] border border-[#2a2a38] rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-3">
              ...
              {(detalleImg || detalle.imagen_thumb) ? (
                ...
                <div className="w-full h-24 rounded-lg border border-dashed border-[#2a2a38] flex items-center justify-center text-[12.5px] text-[#8a8a9c]">Sin imagen</div>
              )}
  ...
  return (
    <div className="space-y-4">
      <div className={card}>
        ...
        <p className="text-[12.5px] text-[#8a8a9c]">Herramientas para pasar de lo que dice una etiqueta o un análisis a las unidades del objetivo (ppm elemental).</p>
      </div>

      {/* 1) Óxido → elemental */}
      <div className={card}>
        <h4 className="text-[13.5px] font-semibold text-[#d9f99d] mb-2">Óxido → Elemental <span className="text-[11.5px] text-[#8a8a9c] font-normal">· leer etiquetas NPK</span></h4>
        <div className="flex flex-wrap items-end gap-2">
          ...
          <span className="text-[#8a8a9c] pb-1.5">=</span>
          <div className="pb-1">{res(oxRes != null ? `${oxRes} % ${ox.a}` : '—')}</div>
        </div>
        <p className="text-[11.5px] text-[#8a8a9c] mt-1.5">{ox.nota}. Factor ×{ox.factor}. Ej: una etiqueta 0-15-25 → P {(+15 * 0.4364).toFixed(1)}% · K {(+25 * 0.8301).toFixed(1)}%.</p>
      </div>

      {/* 2) Concentración */}
      <div className={card}>
        <h4 className="text-[13.5px] font-semibold text-[#d9f99d] mb-2">Concentración <span className="text-[11.5px] text-[#8a8a9c] font-normal">· % ↔ g/L ↔ ppm</span></h4>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-[#15151d] border border-[#1f1f2b] rounded-md px-3 py-2"><p className="text-[10.5px] text-[#8a8a9c]">%</p>{res(+(ppmBase / 10000).toFixed(4))}</div>
          <div className="bg-[#15151d] border border-[#1f1f2b] rounded-md px-3 py-2"><p className="text-[10.5px] text-[#8a8a9c]">g/L</p>{res(+(ppmBase / 1000).toFixed(3))}</div>
          <div className="bg-[#15151d] border border-[#1f1f2b] rounded-md px-3 py-2"><p className="text-[10.5px] text-[#8a8a9c]">ppm (mg/L)</p>{res(+ppmBase.toFixed(1))}</div>
        </div>
        <p className="text-[11.5px] text-[#8a8a9c] mt-1.5">1% = 10 g/L = 10.000 ppm (asume solución acuosa, densidad ~1).</p>
      </div>

      {/* 3) ppm ↔ meq */}
      <div className={card}>
        <h4 className="text-[13.5px] font-semibold text-[#d9f99d] mb-2">Iónico <span className="text-[11.5px] text-[#8a8a9c] font-normal">· ppm ↔ meq/L (análisis de agua)</span></h4>
        <div className="flex flex-wrap items-end gap-2">
        ...
        <p className="text-[11.5px] text-[#8a8a9c] mt-1.5">Peso equivalente {ionElem} = {PESO_EQ[ionElem]} g/eq. Útil para leer dureza/bases de un análisis de agua.</p>
      </div>

      {/* 4) Unidades */}
      <div className={card}>
        <h4 className="text-[13.5px] font-semibold text-[#d9f99d] mb-2">Unidades <span className="text-[11.5px] text-[#8a8a9c] font-normal">· para productos importados</span></h4>
        ...
        <p className="text-[11.5px] text-[#8a8a9c] mt-1.5">1 oz = 28.35 g · 1 galón US = 3.785 L. Ej: una etiqueta yanqui "4 mL/gal" = {(4 / 3.78541).toFixed(2)} mL/L.</p>
      </div>
    </div>
  )
}
...
function RatiosTab({ ratios, res, costo, proveedores }: { ratios: Ratios; res: Resultado; costo: CostoResultado; proveedores: Proveedor[] }) {
  ...
  return (
        ...
        <ul className="space-y-1.5">
          {redundancias.map(r => (
            <li key={r.elemento} className="text-[13px] text-[#d4d4dd]">
              ...
              <span className="text-[#8a8a9c]"> ({r.fuentes.join(' + ')})</span>
            </li>
          ))}
        </ul>
      </div>
    ...
    <div className="grid lg:grid-cols-2 gap-4">
      <div className={card}>
        ...
        <div className="space-y-1.5">
          <div className="bg-[#15151d] border border-[#1f1f2b] rounded-md px-3 py-2">
            ...
            <p className="text-[11.5px] text-[#8a8a9c] mt-0.5">La proporción de los 3 macros principales (siempre relativa al fósforo=1). Es la "firma" NPK de tu fórmula.</p>
          </div>
          {Object.entries(ratios).filter(([k]) => k !== 'N:P:K').map(([k, v]) => {
            ...
            <div key={k} className="bg-[#15151d] border border-[#1f1f2b] rounded-md px-3 py-2">
              <div className="flex items-center justify-between">
                <span className="text-[13px] text-[#a6a6b5]">{k} {info && <span className="text-[11.5px] text-[#8a8a9c]">· ideal {info.ideal}</span>}</span>
                <span className="text-[13.5px] font-mono tabular-nums font-bold text-[#ececf1]">{(v as number) || '—'}</span>
              </div>
              {info && <p className="text-[11.5px] text-[#8a8a9c] mt-0.5">{info.desc}</p>}
            </div>
            )
          })}
        </div>
        <p className="text-[11.5px] text-[#8a8a9c] mt-2">Los ratios te avisan de antagonismos: si un elemento está muy alto respecto a otro, la planta absorbe de menos el segundo aunque sobre.</p>
      </div>
      ...
      <div className={card}>
        ...
        {detalleCosto.length === 0 ? (
          <p className="text-[12.5px] text-[#8a8a9c]">Cargá costo/kg en cada sustancia para ver el costo.</p>
        ) : (
          <div className="space-y-1">
            {detalleCosto.map(d => (
              <div key={d.sal.id} className="flex items-center gap-2 bg-[#15151d] border border-[#1f1f2b] rounded-md px-2.5 py-1.5">
        ...
        <p className="text-[11.5px] text-[#8a8a9c] mt-2">El costo usa el proveedor ⭐ de cada sal, pero podés elegir otro en el desplegable para presupuestar sin cambiar la referencia.</p>
      </div>
    </div>
    ...
    <div className={card}>
      ...
      <p className="text-[11.5px] text-[#8a8a9c] mt-2">{phInfo.d} · Los cationes (⊕ Ca+Mg+K+NH₄) deben igualar a los aniones (⊖ NO₃+SO₄+H₂PO₄+Cl y otros). "otros⁻" = bicarbonato/gluconato/silicato (balancean pero no son nutrientes). Desbalance ≤5% = receta física real.</p>
    </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      <div className={card}>
        ...
        <p className="text-[12.5px] text-[#8a8a9c] mb-3">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <label className="text-[12.5px] text-[#a6a6b5]">Dosis ({prod?.liquido ? 'mL/L' : 'g/L'})
            {DOSIS_REC[id] != null && <span className="text-[#8a8a9c] ml-1">· recomendada de la marca: {DOSIS_REC[id]}{prod?.liquido ? ' mL/L' : ' g/L'} (podés cambiarla)</span>}
        ...
        {prod && (
          <>
            <div className="mt-3 rounded-lg bg-[#15151d] border border-[#1f1f2b] p-3">
              <p className="text-[11.5px] uppercase tracking-[0.12em] text-[#8a8a9c] mb-2">Perfil resultante (ppm a esa dosis)</p>
              <div className="flex flex-wrap gap-1.5">
                ...
                {esAditivo && <span className="text-[12.5px] text-[#8a8a9c]">Este producto no aporta nutrientes (es un aditivo): no se clona con sales.</span>}
              </div>
            </div>
            {esAditivo ? (
...
function FilaProveedor({ costo, volumen }: { costo: CostoSustancia; volumen: number }) {
  ...
  if (!proveedor) {
    return (
      <div className="mt-1.5 pt-1.5 border-t border-[#1f1f2b] text-[11.5px] text-[#8a8a9c]">
        Sin proveedor cargado. Agregalo en <b className="text-[#8a8a9c]">Más herramientas › Proveedores</b> y acá aparece el precio y el rinde.
      </div>
    )
  }
    ...
    <div className="mt-1.5 pt-1.5 border-t border-[#1f1f2b] flex flex-wrap items-baseline gap-x-3 gap-y-1 text-[11.5px]">
      ...
      {envase && precioEnvase != null && (
        <span className="text-[#c4c4d0]">
          {envase} <span className="text-[#8a8a9c]">·</span> <b className="font-mono tabular-nums">{fmtPesos(precioEnvase)}</b>
        </span>
      )}
      {porUnidadBase != null && (
        <span className="text-[#8a8a9c] font-mono tabular-nums">
          ...
          {[...rec.aditivos].sort((a, b) => {
            ...
            return (
              <div key={info.id} className="rounded-md bg-[#15151d] border border-[#1f1f2b] px-3 py-2">
                ...
                <div className="text-[11.5px] text-[#8a8a9c] mt-0.5">{info.funcion} · dosis {info.dosis}</div>
      ...
      <div className={card}>
        ...
        {r && ag && (
          <div className="mt-4 rounded-xl p-4 border border-[#404d20] bg-[#a3e635]/08">
            ...
            {ag.nota && <p className="text-[12px] text-[#8a8a9c] mt-1">{ag.nota}</p>}
          </div>
        )}
...
function CompararTab({ guardados, perfilActual, comerciales }: {
  ...
  return (
    <div className="space-y-4">
      <div className={card}>
        ...
        {hayComercial && (
          <label className="flex flex-wrap items-center gap-2 mt-3">
            ...
            <span className="text-[12.5px] text-[#8a8a9c]">mL/L (o g/L) — la etiqueta suele decir 4</span>
          </label>
        )}
      </div>
          ...
          {desvioMedio != null && (
            <div className={card}>
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                <span className="text-[11.5px] uppercase tracking-[0.12em] text-[#8a8a9c] font-medium">Qué tan parecidas son</span>
                ...
                <span className="text-[12.5px] text-[#8a8a9c]">de desvío promedio</span>
              </div>
          ...
          <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-[12.5px] min-w-[420px]">
                <thead><tr className="border-b border-[#1f1f2b]">
                  <th className="px-3 py-2 text-left text-[11.5px] uppercase tracking-[0.1em] text-[#8a8a9c] font-medium">Elemento</th>
                  ...
                  <th className="px-3 py-2 text-right text-[11.5px] uppercase tracking-[0.1em] text-[#8a8a9c] font-medium">Δ</th>
                </tr></thead>
                <tbody>
                  <tr className="border-b border-[#1f1f2b] bg-[#15151d]">
                    ...
                    <td className="px-3 py-1.5 text-right font-mono tabular-nums text-[#8a8a9c]">{(ecOf(pb.perfil) - ecOf(pa.perfil)).toFixed(2)}</td>
                  </tr>
                  {elementos.map(e => {
                    const va = pa.perfil[e.key] ?? 0, vb = pb.perfil[e.key] ?? 0
                    ...
                    return (
                      <tr key={e.key} className="border-b border-[#1f1f2b] last:border-0">
                        ...
                        <td className="px-3 py-1.5 text-right font-mono tabular-nums"
                          style={{ color: pctDesvio <= 10 ? '#8a8a9c' : d > 0 ? '#60a5fa' : '#ff8a7a' }}>
                          {d > 0 ? '+' : ''}{d || '—'}
                        </td>
                      </tr>
                    )
            ...
            <p className="px-3 py-2 text-[11.5px] text-[#8a8a9c] border-t border-[#1f1f2b]">
      ...
      {!pb && (
        <div className={card}>
          <p className="text-[13.5px] text-[#8a8a9c] py-6 text-center">Elegí en <b className="text-[#a6a6b5]">B</b> contra qué querés comparar.</p>
        </div>
      )}
    </div>
  )
