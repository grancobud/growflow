    ...
    <div className="space-y-4">
      <div className={`${card} p-3 sm:p-4`}>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-[13.5px] text-[#8a8a9c]">
          ...
          <div className="relative ml-auto flex-1 sm:flex-none sm:w-56 min-w-[140px]">
            <Search className="w-3.5 h-3.5 text-[#8a8a9c] absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none" strokeWidth={1.8} />
        ...
        <p className="text-[13.5px] text-[#8a8a9c] mt-2 leading-relaxed">
      ...
      {cargando ? (
        <div className={`${card} p-10 text-center`}>
          <Loader2 className="w-5 h-5 text-[#8a8a9c] animate-spin mx-auto" />
        </div>
      ) : porMarca.length === 0 ? (
        <div className={`${card} p-10 text-center`}>
        ...
        <div key={marca} className="space-y-2">
          <div className="flex items-baseline gap-2 px-0.5">
            ...
            <span className="text-[13px] text-[#8a8a9c]">{items.length} producto{items.length === 1 ? '' : 's'}</span>
          </div>
    ...
    <div className={`${card} p-3.5 flex flex-col gap-2.5`}>
      <div className="flex items-start gap-2">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[13px] text-[#8a8a9c] mt-0.5">
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        <span className="text-[12.5px] text-[#8a8a9c] font-mono tabular-nums">{total}% declarado</span>
      </div>
        ...
        <div className="flex flex-wrap gap-1">
          {ELEMENTOS.filter(e => (f.composicion[e.key] ?? 0) > 0).map(e => (
            <span key={e.key} className="text-[12.5px] font-mono tabular-nums px-1.5 py-0.5 rounded bg-[#15151d] border border-[#1f1f2b] text-[#a6a6b5]">
              <span className="text-[#8a8a9c]">{e.key}</span> {f.composicion[e.key]}
            </span>
          ))}
        </div>
      )}
      ...
      {f.sales_origen.length > 0 && (
        <div>
          <p className="text-[12px] uppercase tracking-[0.1em] text-[#8a8a9c] mb-1">Sale de</p>
      ...
      <div className="flex items-center gap-1.5 flex-wrap mt-auto pt-1">
        {f.pdf_path ? (
          <button onClick={verPdf} disabled={abriendo}
            className="flex items-center gap-1.5 text-[13.5px] px-2.5 py-2 min-h-[44px] rounded-md bg-[#15151d] border border-[#1f1f2b] text-[#a6a6b5] hover:text-[#c4b5fd] hover:border-[#463a66] transition-colors disabled:opacity-50">
            {abriendo ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ExternalLink className="w-3.5 h-3.5" strokeWidth={1.8} />}
            Ver ficha <span className="text-[#8a8a9c]">{pesoLegible(f.pdf_tam)}</span>
          </button>
        ) : (
          <span className="text-[13px] text-[#8a8a9c] px-1">Sin archivo adjunto</span>
        )}
        {onClonar && Object.keys(f.composicion).length > 0 && (
          <button onClick={() => onClonar(perfilDesdeFicha(f, dosisRef), `${f.marca} ${f.producto}`)}
            ...
            className="flex items-center gap-1.5 text-[13.5px] px-2.5 py-2 min-h-[44px] rounded-md bg-[#a3e635]/10 border border-[#404d20] text-[#d9f99d] hover:bg-[#a3e635]/20 transition-colors">
            <FlaskConical className="w-3.5 h-3.5" strokeWidth={1.8} /> Clonar
          </button>
        )}
        <button onClick={onEditar} aria-label={`Editar ${f.producto}`}
          className="ml-auto p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-md text-[#8a8a9c] hover:text-[#d9f99d] hover:bg-[#a3e635]/10 transition-colors">
          <Pencil className="w-3.5 h-3.5" strokeWidth={1.8} />
        </button>
        <button onClick={onBorrar} aria-label={`Borrar ${f.producto}`}
          className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-md text-[#8a8a9c] hover:text-[#ff8a7a] hover:bg-[#ff8a7a]/10 transition-colors">
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <div className="sticky top-0 bg-[#0d0d12] border-b border-[#1f1f2b] px-4 py-3 flex items-center gap-2 z-10">
          ...
          <button onClick={onCerrar} aria-label="Cerrar"
            className="ml-auto p-2 rounded-md text-[#8a8a9c] hover:text-[#ececf1] hover:bg-[#1f1f2b] transition-colors">
        ...
        <div className="p-4 space-y-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
          ...
          <div>
            <div className="flex items-baseline gap-2 mb-2">
              <p className="text-[12.5px] uppercase tracking-[0.12em] text-[#8a8a9c] font-medium">Composición</p>
              <span className="text-[13px] text-[#8a8a9c]">en % p/p, como lo dice la etiqueta</span>
          ...
          <label className="block"><span className={lbl}>Sale de estas sales <span className="text-[#8a8a9c]">— separadas por coma</span></span>
          ...
          <label className="block"><span className={lbl}>Nota <span className="text-[#8a8a9c]">— qué encontraste al analizarla</span></span>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-[13px] text-[#8a8a9c]">— las sales declaradas dan los porcentajes declarados</span>
          </label>

          <div>
            <p className={lbl}>Ficha / etiqueta <span className="text-[#8a8a9c]">— PDF o foto, hasta {PDF_MAX_MB} MB</span></p>
            <div className="flex items-center gap-2 flex-wrap">
