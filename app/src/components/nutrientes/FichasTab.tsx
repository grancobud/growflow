...
const inp = 'w-full bg-[#15151d] border border-[#1f1f2b] rounded-md px-2.5 py-2 min-h-[42px] text-[16px] sm:text-[13px] text-[#ececf1] placeholder:text-[#4a4a58] focus:border-[#404d20] outline-none'
const lbl = 'block text-[11px] text-[#8f8f9f] mb-1'
    ...
    <div className="space-y-4">
      <div className={`${card} p-3 sm:p-4`}>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <h3 className="font-display font-semibold text-[15px] text-[#ececf1]">Fichas técnicas</h3>
          <span className="text-[12px] text-[#5c5c6b]">
          ...
          <button onClick={() => setEditando('nueva')}
            className="flex items-center gap-1.5 px-3 py-2 min-h-[42px] rounded-md text-[13px] font-medium bg-[#a3e635]/15 border border-[#404d20] text-[#d9f99d] hover:bg-[#a3e635]/25 transition-colors flex-shrink-0">
            <Plus className="w-4 h-4" strokeWidth={2} /> Nueva ficha
          </button>
        </div>
        <p className="text-[12px] text-[#5c5c6b] mt-2 leading-relaxed">
      ...
      {cargando ? (
        ...
        <div className={`${card} p-10 text-center`}>
          ...
          <p className="text-[14px] text-[#8f8f9f]">
        ...
        <div key={marca} className="space-y-2">
          <div className="flex items-baseline gap-2 px-0.5">
            <h4 className="font-display font-semibold text-[14px] text-[#d4d4dd]">{marca}</h4>
            <span className="text-[11.5px] text-[#5c5c6b]">{items.length} producto{items.length === 1 ? '' : 's'}</span>
          </div>
    ...
    <div className={`${card} p-3.5 flex flex-col gap-2.5`}>
      <div className="flex items-start gap-2">
        <div className="min-w-0 flex-1">
          <h5 className="font-display font-semibold text-[14px] text-[#ececf1] leading-tight">{f.producto}</h5>
          <p className="text-[11.5px] text-[#5c5c6b] mt-0.5">
            {[f.linea, f.forma === 'liquido' ? `líquido · ${f.densidad ?? 1} g/mL` : 'polvo'].filter(Boolean).join(' · ')}
          </p>
        </div>
        {f.npk && (
          <span className="text-[12px] font-mono tabular-nums px-2 py-1 rounded border border-[#2a2a3a] bg-[#15151d] text-[#a6a6b5] flex-shrink-0">
            {f.npk}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {f.verificado ? (
          <span className="inline-flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded bg-[#a3e635]/10 border border-[#404d20] text-[#d9f99d]">
            <CheckCircle2 className="w-3 h-3" strokeWidth={2} /> Cierra la cuenta
          </span>
        ) : (
          <span className="inline-flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded bg-[#f59e0b]/10 border border-[#78500f] text-[#fcd34d]">
            <AlertTriangle className="w-3 h-3" strokeWidth={2} /> La etiqueta no cierra
          </span>
        )}
        <span className="text-[11px] text-[#5c5c6b] font-mono tabular-nums">{total}% declarado</span>
      </div>
      ...
      {Object.keys(f.composicion).length > 0 && (
        <div className="flex flex-wrap gap-1">
          {ELEMENTOS.filter(e => (f.composicion[e.key] ?? 0) > 0).map(e => (
            <span key={e.key} className="text-[11px] font-mono tabular-nums px-1.5 py-0.5 rounded bg-[#15151d] border border-[#1f1f2b] text-[#a6a6b5]">
      ...
      {f.sales_origen.length > 0 && (
        <div>
          <p className="text-[10.5px] uppercase tracking-[0.1em] text-[#5c5c6b] mb-1">Sale de</p>
          <p className="text-[12px] text-[#a6a6b5] leading-relaxed">{f.sales_origen.join(' · ')}</p>
        </div>
      )}

      {f.nota && <p className="text-[12px] text-[#8f8f9f] leading-relaxed">{f.nota}</p>}

      <div className="flex items-center gap-1.5 flex-wrap mt-auto pt-1">
        {f.pdf_path ? (
          <button onClick={verPdf} disabled={abriendo}
            className="flex items-center gap-1.5 text-[12px] px-2.5 py-2 min-h-[38px] rounded-md bg-[#15151d] border border-[#1f1f2b] text-[#a6a6b5] hover:text-[#c4b5fd] hover:border-[#463a66] transition-colors disabled:opacity-50">
          ...
          <span className="text-[11.5px] text-[#3a3a4a] px-1">Sin archivo adjunto</span>
        )}
        {onClonar && Object.keys(f.composicion).length > 0 && (
          <button onClick={() => onClonar(perfilDesdeFicha(f, dosisRef), `${f.marca} ${f.producto}`)}
            ...
            className="flex items-center gap-1.5 text-[12px] px-2.5 py-2 min-h-[38px] rounded-md bg-[#a3e635]/10 border border-[#404d20] text-[#d9f99d] hover:bg-[#a3e635]/20 transition-colors">
    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onCerrar}>
      <div onClick={e => e.stopPropagation()}
        ...
        <div className="sticky top-0 bg-[#0d0d12] border-b border-[#1f1f2b] px-4 py-3 flex items-center gap-2 z-10">
          ...
          <h3 className="font-display font-semibold text-[15px] text-[#ececf1]">
        ...
        <div className="p-4 space-y-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
          ...
          <div>
            <div className="flex items-baseline gap-2 mb-2">
              <p className="text-[11px] uppercase tracking-[0.12em] text-[#5c5c6b] font-medium">Composición</p>
              <span className="text-[11.5px] text-[#3a3a4a]">en % p/p, como lo dice la etiqueta</span>
              <span className="ml-auto text-[12px] font-mono tabular-nums text-[#a3e635]/70">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
              {ELEMENTOS.map(e => (
                <label key={e.key}>
                  <span className="block text-[11px] text-[#8f8f9f] mb-0.5">{e.key}</span>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-[13px] text-[#d4d4dd]">La estequiometría cierra</span>
            <span className="text-[11.5px] text-[#5c5c6b]">— las sales declaradas dan los porcentajes declarados</span>
          </label>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button onClick={() => fileRef.current?.click()}
                className="flex items-center gap-1.5 px-3 py-2 min-h-[42px] rounded-md text-[13px] bg-[#15151d] border border-[#1f1f2b] text-[#a6a6b5] hover:text-[#c4b5fd] hover:border-[#463a66] transition-colors">
              ...
              <span className="text-[12px] text-[#8f8f9f] truncate">
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={onCerrar}
              className="flex-1 px-3 py-2.5 min-h-[44px] rounded-md text-[13px] bg-[#15151d] border border-[#1f1f2b] text-[#a6a6b5] hover:text-[#ececf1] transition-colors">
              Cancelar
            </button>
            <button onClick={guardar} disabled={guardando}
              className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 min-h-[44px] rounded-md text-[13px] font-medium bg-[#a3e635]/15 border border-[#404d20] text-[#d9f99d] hover:bg-[#a3e635]/25 transition-colors disabled:opacity-50">
