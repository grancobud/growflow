    ...
    return (
      <button key={p.id} onClick={() => clickPlanta(p)}
        ...
        {genNombre ? (
          ...
          <span className="text-[9px] leading-none text-[#8a8a9c]">sin gen.</span>
        )}
      </button>
    )
  }
    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0a0f] text-[#d4d4dd] font-sans">
      <div className="sticky top-0 z-40 bg-[#0a0a0f]/95 backdrop-blur-[2px] border-b border-[#1f1f2b]">
            ...
            {([['regar', Droplets, 'Regar'], ['fumigar', SprayCan, 'Fumigar'], ['mover', Move, 'Mover'], ['genetica', Dna, 'Genética']] as const).map(([m, Ic, lbl]) => (
              <button key={m} onClick={() => { setModo(m); setMoviendo(null) }}
                className={`inline-flex items-center justify-center gap-1.5 px-3 py-1.5 min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 text-[11.5px] font-medium transition-colors ${
                  modo === m ? 'bg-[#a3e635]/15 text-[#d9f99d]' : 'bg-[#15151d] text-[#8a8a9c] hover:text-[#a6a6b5]'
                }`}>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 px-3 sm:px-6 pb-2.5 text-[10px] text-[#8a8a9c]">
          ...
          <span className="text-[#8a8a9c]">· puntito de color = genética</span>
        ...
        {modo === 'regar' && (
          <div className="flex items-center flex-wrap gap-2 px-3 sm:px-6 pb-2.5">
            <span className="text-[10px] uppercase tracking-[0.12em] text-[#8a8a9c] font-medium">Receta de hoy:</span>
            {([['volumen', 'Volumen ml', '2000'], ['ppm', 'PPM', '800'], ['ph', 'pH', '6.2'], ['escurrido', 'Escurrido ml', '300']] as const).map(([k, ph, eg]) => (
              <input key={k} type="number" inputMode="decimal" value={receta[k]}
                ...
                className="w-[120px] px-2 py-1 rounded-lg bg-[#15151d] border border-[#2a2a3a] text-[16px] sm:text-[11px] text-[#ececf1] placeholder-[#8a8a9c] focus:outline-none focus:border-[#38bdf8]/60 min-h-[44px] sm:min-h-0" />
            ))}
            <span className="text-[10px] text-[#8a8a9c]">se guarda en cada planta que toques</span>
          </div>
        )}
        {/* Receta de fumigacion/aplicacion */}
        {modo === 'fumigar' && (
          <div className="flex items-center flex-wrap gap-2 px-3 sm:px-6 pb-2.5">
            <span className="text-[10px] uppercase tracking-[0.12em] text-[#8a8a9c] font-medium">Aplicación:</span>
          ...
          {CARPAS.map(carpa => {
            ...
            <div key={carpa.id} style={{ gridArea: carpa.area, maxWidth: carpa.cols * 57 + 20 }}
              ...
              <h2 className="flex items-center justify-between gap-2 text-[11px] font-semibold text-[#8a8a9c] mb-2 whitespace-nowrap">
                <span className="truncate min-w-0">{carpa.nombre} <span className="font-normal text-[#8a8a9c]">{carpa.medida}</span></span>
                {modo === 'regar' && (
                  <button onClick={() => regarCarpa(carpa)}
                    className="inline-flex items-center justify-center gap-1 px-2 py-1.5 sm:px-1.5 sm:py-0.5 min-h-[44px] sm:min-h-0 rounded border border-[#1d3a5a] bg-[#38bdf8]/10 hover:bg-[#38bdf8]/20 text-[9.5px] text-[#38bdf8] transition-colors"
