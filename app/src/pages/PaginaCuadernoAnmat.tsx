    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0a0f] text-[#d4d4dd] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0a0f]/95 backdrop-blur-[2px] border-b border-[#1f1f2b] print:hidden">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#8a8a9c]">ANMAT Res 1780/2025 · {filtradas.length} actividad{filtradas.length === 1 ? '' : 'es'} {camada ? `de ${camada}` : ''}</div>
          </div>
        ...
        <div className="px-3 sm:px-6 pb-3">
          <div className="flex flex-wrap items-end gap-2 sm:gap-3">
            <div>
              <label className="block text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1">Camada</label>
              <select
                value={camada}
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1">Desde</label>
              <input type="date" value={desde} onChange={e => setDesde(e.target.value)}
                className="px-3 py-1.5 bg-[#101016] border border-[#1f1f2b] rounded-lg text-xs text-[#d4d4dd] focus:outline-none focus:ring-1 focus:ring-[#a3e635]/30" />
            </div>
            <div>
              <label className="block text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1">Hasta</label>
              <input type="date" value={hasta} onChange={e => setHasta(e.target.value)}
                className="px-3 py-1.5 bg-[#101016] border border-[#1f1f2b] rounded-lg text-xs text-[#d4d4dd] focus:outline-none focus:ring-1 focus:ring-[#a3e635]/30" />
            </div>
            <div>
              <label className="block text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1">Categoria</label>
              <select
                value={categoria}
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-3 print:p-0">
        ...
        {cargando && entradas.length === 0 && (
          <div className="flex items-center justify-center py-16 text-[#8a8a9c] gap-2">
            <RefreshCcw className="w-4 h-4 animate-spin" /> Cargando cuaderno...
          </div>
        )}

        {!cargando && filtradas.length === 0 && (
          <div className="text-center py-16 text-[#8a8a9c]">
        ...
        {porDia.map(([dia, actividades]) => (
          ...
          >
            <div className="px-4 py-2.5 border-b border-[#1f1f2b] flex items-center justify-between">
              ...
              <span className="text-[10px] text-[#8a8a9c] tabular-nums">{actividades.length} actividad{actividades.length === 1 ? '' : 'es'}</span>
            </div>
                ...
                return (
                  <div key={a.id} className="px-4 py-2.5 flex items-start gap-3 text-[12px]">
                    ...
                    <div className="flex-1 min-w-0 space-y-0.5">
                      ...
                      {a.observaciones && <p className="text-[10px] text-[#8a8a9c] italic">{a.observaciones}</p>}
                    </div>
                    <div className="text-right text-[10px] text-[#8a8a9c] flex-shrink-0 space-y-0.5">
