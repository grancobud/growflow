  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#0a0a0f] text-[#d4d4dd] font-sans">
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        {/* Loading spinner */}
        {cargando && (
          <div className="py-12 text-center text-[#8a8a9c]">
          ...
          <>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2.5">
                <span className="text-[10px] uppercase tracking-[0.16em] text-[#8a8a9c] font-medium">Promedios historicos</span>
              </div>
              <motion.div
            ...
            <div>
              <div className="flex items-center gap-2 mb-2.5">
                <span className="text-[10px] uppercase tracking-[0.16em] text-[#8a8a9c] font-medium">Proyeccion — camadas en curso</span>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2.5">
                <span className="text-[10px] uppercase tracking-[0.16em] text-[#8a8a9c] font-medium">Historicos por camada + sistema</span>
              </div>
              ...
              {proyecciones.length === 0 ? (
                <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] p-8 text-center">
                  ...
                  <p className="text-[11px] text-[#8a8a9c] mt-1">Carga cosechas desde /operacion para ir acumulando data historica.</p>
                </div>
              ) : (
                    ...
                    <thead className="border-b border-[#1f1f2b]">
                      <tr>
                        {['Camada','Sistema','N°','Ø Plantas','Ø Fresco','Ø Seco','Ø g/pl fresco','Ø g/pl seco','Ø Rend.','Ultima'].map(h => (
                          <th key={h} className="px-3 py-2.5 text-left text-[9.5px] font-semibold uppercase tracking-[0.12em] text-[#8a8a9c] whitespace-nowrap first:text-left last:text-left text-right first:[&]:text-left">
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="divide-y divide-[#1f1f2b]">
                      {proyecciones.map(p => (
                        <tr key={`${p.camada}-${p.sistema}`} className="hover:bg-[#15151d] transition-colors">
                          ...
                          <td className="px-3 py-2 text-[#8a8a9c] font-mono tabular-nums">{p.ultima_cosecha?.slice(0, 10) ?? '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-[#8a8a9c]">{label}</p>
...
function ProyeccionCamada({
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#8a8a9c]">
              {sinDataPropia ? 'Sin historicos propios — promedio general' : 'Basado en historicos propios'}
            </p>
          </div>
        </div>
        <span className="text-[10px] text-[#8a8a9c] flex items-center gap-1 font-mono">
      ...
      <div className="mb-3">
        <label className="block text-[10px] font-semibold uppercase tracking-[0.12em] text-[#8a8a9c] mb-1">
          Plantas esperadas
        </label>
        <input
          type="number"
