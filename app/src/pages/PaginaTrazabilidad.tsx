...
function LotCode({ code, size = 'sm', dim = false }: { code: string; size?: 'xs' | 'sm' | 'md'; dim?: boolean }) {
  ...
  return (
    <span className={`font-mono tabular-nums tracking-tight ${cls} ${dim ? 'text-[#8a8a9c]' : 'text-[#8f8f9f]'} inline-flex flex-wrap items-baseline leading-[1.4]`}>
      {parts.map((p, i) => {
            ...
            <AnimatePresence>{tip === i && (
              <motion.span initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
                ...
                <span className="block px-2.5 py-1.5 rounded-md bg-[#0a0a0f] border border-[#2b2b3a] shadow-[0_8px_24px_rgba(0,0,0,0.6)]">
                  <span className="block text-[9px] uppercase tracking-widest text-[#8a8a9c] font-sans font-medium mb-0.5">{tok.label}</span>
...
function Label({ children, tone = 'ink-40', className = '' }: { children: React.ReactNode; tone?: 'ink-40' | 'ink-50' | 'gold' | 'primary'; className?: string }) {
  const map = { 'ink-40': 'text-[#8a8a9c]', 'ink-50': 'text-[#8a8a9c]', gold: 'text-[#a78bfa]', primary: 'text-[#bef264]' }
    ...
    <div className="w-full">
      {showLabel && (
        <div className="flex items-baseline justify-between mb-1.5">
          <span className="font-mono tabular-nums text-[11px] text-[#8a8a9c]">
            <span className={completed ? 'text-[#c4b5fd]' : 'text-[#d9f99d]'}>{current}</span>
            <span className="text-[#8a8a9c]">/{total}</span>
            <span className="text-[#8a8a9c] mx-1.5">·</span>
            <span className="text-[#b3b3c0] font-medium"><CountUp to={pct} />%</span>
          </span>
          <span className="text-[10px] uppercase tracking-[0.14em] font-medium text-[#8a8a9c]">{completed ? 'Completada' : 'En progreso'}</span>
        </div>
      )}
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="font-mono tabular-nums text-[12px] font-semibold text-[#ececf1]">{current}<span className="text-[#8a8a9c]">/{total}</span></span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] px-3 sm:px-5 py-3 sm:py-4 flex items-center gap-2 sm:gap-4 flex-wrap">
      <div className="min-w-0">
        ...
        <div className="mt-0.5 text-[11.5px] text-[#8a8a9c]">
  ...
  return (
    <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] overflow-hidden">
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center gap-3 px-5 py-4 hover:bg-[#15151d] transition-colors text-left">
        <motion.span animate={{ rotate: open ? 90 : 0 }} transition={{ duration: 0.2, ease: EASE }} className="text-[#8a8a9c]">
        ...
        <span className="text-[12px] text-[#8a8a9c] hidden md:inline">Cómo se forman los códigos de trazabilidad</span>
      </button>
      <AnimatePresence initial={false}>{open && (
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
          ...
          <div className="px-5 py-5">
            <div className="text-[10px] uppercase tracking-[0.16em] text-[#8a8a9c] font-medium mb-2 text-center">Planta individual · vegetativo y floración</div>
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-6">
              {example.map((seg, i) => (
                <div key={i} className="rounded-md bg-[#15151d] border border-[#20202c] px-3 py-3 flex flex-col items-center justify-between min-h-[78px]">
                  <div className="text-[9px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium text-center leading-tight">{seg.l}</div>
                  <div className={`my-1 font-mono font-semibold text-[12px] text-center ${colorMap[seg.c].split(' ')[2]}`}>{seg.v}</div>
                  <div className="text-[10px] text-[#8a8a9c] leading-tight text-center">{seg.d}</div>
                </div>
              ))}
            </div>
            <div className="text-[10px] uppercase tracking-[0.16em] text-[#8a8a9c] font-medium mb-2 text-center">Post-cosecha · cuadro de secado</div>
            <div className="flex justify-center mb-4">
              <div className="font-mono tabular-nums text-[13px] sm:text-[18px] tracking-tight flex items-baseline flex-wrap justify-center gap-y-1">
            ...
            <div className="text-[10px] uppercase tracking-[0.16em] text-[#8a8a9c] font-medium mb-2 text-center">Fraccionamiento · lote almacén + bolsas</div>
            <div className="flex justify-center mb-4">
              <div className="font-mono tabular-nums text-[13px] sm:text-[18px] tracking-tight flex items-baseline flex-wrap justify-center gap-y-1">
            ...
            <div className="text-[10px] uppercase tracking-[0.16em] text-[#8a8a9c] font-medium mb-2 text-center">Depósito final · bolsa comercial</div>
            <div className="flex justify-center mb-4">
              <div className="font-mono tabular-nums text-[13px] sm:text-[18px] tracking-tight flex items-baseline flex-wrap justify-center gap-y-1">
            ...
            <div className="rounded-lg bg-[#0a0a0f] border border-[#1f1f2b] p-4 mb-5">
              <div className="text-[10px] uppercase tracking-[0.16em] text-[#8a8a9c] font-medium mb-3">Código por etapa del ciclo</div>
              <div className="flex flex-col gap-1.5">
                {patterns.map((p, i) => (
                  <div key={p.stage} className="flex items-baseline gap-3 py-0.5">
                    <span className="font-mono text-[#8a8a9c] text-[11px] w-[18px] shrink-0 text-right">{i + 1}.</span>
                    ...
                    {p.note && <span className="text-[10px] text-[#8a8a9c] italic">{p.note}</span>}
                  </div>
                ))}
              </div>
            </div>
            <div className="mt-5 pt-4 border-t border-[#1f1f2b] space-y-5">
              {[
                ...
                <div key={g.titulo}>
                  <div className="text-[10px] uppercase tracking-[0.16em] text-[#8a8a9c] font-medium mb-1">{g.titulo}</div>
                  <div className="text-[11px] text-[#8a8a9c] mb-2.5 leading-snug">{g.desc}</div>
                  <div className="flex flex-col gap-1.5">
                    {g.items.map(([k, v]) => (
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      <div className="inline-flex gap-1 p-1 rounded-lg bg-[#101016] border border-[#1f1f2b]">
        {btn(filter === 'todas', () => setFilter('todas'), <>Todas <span className={filter === 'todas' ? 'opacity-70' : 'text-[#8a8a9c]'}>({counts.todas})</span></>)}
        {btn(filter === 'proceso', () => setFilter('proceso'), <>En proceso <span className={filter === 'proceso' ? 'opacity-70' : 'text-[#8a8a9c]'}>({counts.proceso})</span></>)}
        {btn(filter === 'completadas', () => setFilter('completadas'), <>Completadas <span className={filter === 'completadas' ? 'opacity-70' : 'text-[#8a8a9c]'}>({counts.completadas})</span></>)}
      </div>
      <div className="inline-flex gap-1 p-1 rounded-lg bg-[#101016] border border-[#1f1f2b]">
    ...
    <motion.button type="button" onClick={() => onSelect(chain.id)}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex items-center gap-1 text-[11px] text-[#8a8a9c]">
    ...
    <motion.li initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="min-w-0">
        <div className="flex items-baseline gap-2">
          <span className="font-mono tabular-nums text-[10.5px] text-[#8a8a9c]">{num}</span>
          <h4 className={`font-display font-semibold text-[13.5px] ${isPending ? 'text-[#8a8a9c]' : 'text-[#ececf1]'}`}>{stageLabel}</h4>
          {isActive && <Pill tone="primary">en curso</Pill>}
          {clickable && <span className="text-[9px] text-[#8a8a9c] uppercase tracking-widest ml-1">click ver detalle</span>}
        </div>
        {stage.code ? <div className="mt-1"><LotCode code={stage.code} /></div> : <div className="mt-1 text-[11px] text-[#30303e]">sin registro</div>}
        {stage.detail && <div className="mt-1 text-[11.5px] text-[#8a8a9c]">{stage.detail}</div>}
        {stage.range && <div className="mt-1 font-mono tabular-nums text-[10.5px] text-[#8a8a9c]">{stage.range}</div>}
      </div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.97, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 20 }}
          ...
          <div className="px-4 sm:px-6 py-3.5 sm:py-5 border-b border-[#1f1f2b] bg-[#15151d] relative">
            ...
            <div className="flex items-baseline gap-2 sm:gap-3 flex-wrap pr-10 sm:pr-0 mt-1.5 sm:mt-0">
              <span className="font-mono tabular-nums text-[10.5px] sm:text-[11px] text-[#8a8a9c]">STAGE {num}</span>
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5 ct-page-scroll">
            ...
            {meta.tipo === 'planta_madre' && (
              <section>
                ...
                <div className="mt-3 text-[11px] text-[#8a8a9c] italic">
                  ...
                  <section>
                    ...
                    <div className="mt-2 px-3 py-2 rounded-md bg-[#15151d] border border-[#1f1f2b] font-mono text-[11px] text-[#d4d4dd]">
                      ...
                      {meta.codigosFmt.last && (
                        <>
                          <div className="text-[#8a8a9c] my-0.5">↓ <span className="not-italic text-[#8a8a9c]">+{Math.max(0, meta.codigosFmt.count - 2)} códigos intermedios</span></div>
                          <div className="text-[#c4b5fd]">{meta.codigosFmt.last}</div>
                        </>
                      )}
                    </div>
                    <div className="mt-2 text-[11px] text-[#8a8a9c]">
            ...
            {meta.tipo === 'cosecha' && (
              <section>
                ...
                <div className="mt-2 text-[11px] text-[#8a8a9c]">Cosecha no genera traza nueva: las plantas mantienen su código de floración.</div>
              </section>
            )}
                    ...
                    {meta.calidad.slice(0, 8).map((c: any) => (
                      <div key={c.id} className="px-2 py-1.5 bg-[#15151d] rounded border border-[#1f1f2b]">
                        <div className="flex items-baseline gap-2">
                          ...
                          <span className="font-mono text-[10px] text-[#8a8a9c]">{c.fecha}</span>
                          {c.resultado && <Pill tone={c.resultado === 'aprobado' ? 'ok' : 'warn'}>{c.resultado}</Pill>}
                        </div>
                        {c.datos_extra?.lote_codigo && <div className="mt-0.5 font-mono text-[10px] text-[#8a8a9c]">lote: {c.datos_extra.lote_codigo}</div>}
                      </div>
                    ))}
                  </div>
                )}
              ...
              <section>
                ...
                {meta.trazas?.length > 0 && (
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-1.5 text-[11px] font-mono text-[#b3b3c0]">
                    ...
                    {meta.trazas.length > 12 && <div className="px-2 py-1 text-[#8a8a9c] italic">...y {meta.trazas.length - 12} más</div>}
                  </div>
                )}
              </section>
            )}
          ...
          <div className="px-6 py-3 border-t border-[#1f1f2b] bg-[#15151d] flex items-center justify-between">
            <div className="text-[10.5px] text-[#8a8a9c] font-mono">Stage {num} · {stageLabel}</div>
    ...
    <div className="mt-2 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
      {stats.map(s => (
        <div key={s.key} className="px-3 py-2 rounded-md bg-[#15151d] border border-[#1f1f2b]">
          <div className="text-[9.5px] uppercase tracking-widest text-[#8a8a9c]">{s.label}</div>
          <div className="mt-0.5 font-mono tabular-nums text-[11.5px]">
            <span className="text-[#c4b5fd] font-semibold">{s.avg}</span><span className="text-[#8a8a9c]"> {s.unit}</span>
          </div>
          <div className="text-[9.5px] font-mono tabular-nums text-[#8a8a9c]">{s.min} – {s.max}</div>
        </div>
      ))}
    </div>
  )
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <h2 className="font-display font-bold tracking-tight text-[22px] text-[#ececf1] leading-none">
              Camada <span className="text-[#d9f99d]">{chain.id}</span><span className="text-[#8a8a9c] font-medium ml-1.5">· {chain.system}</span>
            </h2>
            <div className="mt-1.5 text-[12px] text-[#8a8a9c]">
              ...
              {chain.start && (
                <> · <span className="font-mono tabular-nums text-[#b3b3c0]">{chain.start}</span>
                  {chain.end && chain.end !== chain.start
                    ...
                    : (chain.completed ? null : <span className="text-[#8a8a9c]"> → vigente</span>)}
                </>
              )}
  ...
  return (
    <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] overflow-hidden">
      <div className="px-5 py-3.5 border-b border-[#1f1f2b] flex items-baseline justify-between">
        ...
        <span className="text-[10.5px] text-[#8a8a9c] font-mono">{completed.length} camadas</span>
      </div>
          ...
          return (
            <div key={c.id}>
              <div className="flex items-baseline justify-between mb-1">
                ...
                <span className="font-mono tabular-nums text-[11.5px] text-[#c4b5fd] font-medium">{(c.yieldKg || 0).toFixed(1)}<span className="text-[#8a8a9c] text-[10px] ml-0.5">kg</span></span>
              </div>
              ...
              <div className="mt-1 flex items-center justify-between text-[10.5px]">
                <span className="text-[#8a8a9c] font-mono">{c.system} · {c.start.slice(0, 7)}</span>
                {delta !== null && (
      ...
      <div className="divide-y divide-[#1f1f2b]">
        {items.map((k, i) => (
          <div key={i} className="px-5 py-3 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium">{k.label}</div>
              <div className="mt-0.5 text-[11px] text-[#8a8a9c]">{k.hint}</div>
            </div>
            <div className="font-display font-bold tracking-tight text-[20px] text-[#ececf1] leading-none text-right shrink-0">
              {k.valueText !== undefined ? <span>{k.valueText}</span> : <><CountUp to={k.value || 0} decimals={k.decimals || 0} />{k.suffix && <span className="text-[#8a8a9c] font-medium text-[13px]">{k.suffix}</span>}</>}
            </div>
          </div>
        ))}
      </div>
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto ct-page-scroll bg-[#0a0a0f] text-[#d4d4dd] font-sans">
        ...
        <div className="px-3 sm:px-6 py-3 sm:py-5 space-y-3 sm:space-y-4 pb-20">
                  ...
                  {filtered.length === 0 && (
                    <div className="col-span-full rounded-xl border border-dashed border-[#20202c] bg-[#15151d]/50 px-6 py-10 text-center">
                      <div className="mx-auto w-9 h-9 rounded-full bg-[#1c1c27] border border-[#20202c] flex items-center justify-center mb-2">
                        <svg width="16" height="16" viewBox="0 0 18 18" fill="none"><path d="M9 3v12M3 9h12" stroke="currentColor" strokeWidth="1.2" className="text-[#8a8a9c]" /></svg>
                      </div>
                      <div className="font-display font-semibold text-[#d4d4dd] text-[13px]">Sin cadenas</div>
                      <div className="mt-0.5 text-[11px] text-[#8a8a9c]">Ajustá los filtros.</div>
                    </div>
                  )}
                </div>
          ...
          <footer className="pt-6 mt-2 border-t border-[#1f1f2b] text-[11px] text-[#8a8a9c] flex items-center justify-between flex-wrap gap-2">
        ...
        <Drawer.Content className="fixed z-50 bg-[#101016] border-l border-[#1f1f2b] flex flex-col right-0 top-0 bottom-0 w-full sm:max-w-xl outline-none">
          ...
          <div className="flex items-center justify-between p-5 border-b border-[#1f1f2b] sticky top-0 bg-[#101016] z-10">
            <div>
              ...
              <p className="text-[11.5px] text-[#8a8a9c] mt-0.5">Crear lote raíz (planta madre) e iniciar la trazabilidad.</p>
            </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                autoComplete="off"
                className="w-full px-3 py-2 bg-[#0a0a0f] border border-[#1f1f2b] rounded-md text-[13px] font-mono tabular-nums text-[#ececf1] placeholder:text-[#8a8a9c] focus:outline-none focus:border-[#a3e635]/60"
              />
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 bg-[#0a0a0f] border border-[#1f1f2b] rounded-md text-[13px] text-[#d4d4dd] placeholder:text-[#8a8a9c] focus:outline-none focus:border-[#a3e635]/60 resize-none"
              />
