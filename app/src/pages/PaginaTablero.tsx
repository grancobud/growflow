  ...
  return (
    <div className="flex-1 overflow-y-auto overflow-x-hidden bg-[#0a0a0f] text-[#d4d4dd] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0a0f]/95 backdrop-blur-[2px] border-b border-[#1f1f2b]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3 w-full">
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 w-full">
        {tab === 'doc' && (
...
function DocTab(p: any) {
  ...
  if (!tableros.length) return (
    <div className="text-center py-16">
      ...
      <p className="text-[13px] text-[#8a8a9c] mb-4">No hay tableros cargados todavía.</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={onNuevoTablero} className="px-2.5 py-1.5 rounded-lg text-[12px] border border-dashed border-[#2f2f3d] text-[#8a8a9c] hover:text-[#a3e635] hover:border-[#a3e635]/40">
      ...
      {sel && (
        <>
          ...
          <div className="bg-[#101016] border border-[#1f1f2b] rounded-xl p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="mt-1 text-[11.5px] text-[#8a8a9c] flex flex-wrap gap-x-3 gap-y-0.5">
                ...
                {sel.notas && <div className="mt-1 text-[11px] text-[#8a8a9c] italic">{sel.notas}</div>}
              </div>
              <div className="flex gap-1">
                <button onClick={() => onEditTablero(sel)} className="min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 p-1.5 text-[#8a8a9c] hover:text-[#a3e635]"><Pencil className="w-3.5 h-3.5" /></button>
                <button onClick={() => onBorrarTablero(sel.id)} className="min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 p-1.5 text-[#8a8a9c] hover:text-[#ef4444]"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          ...
          {!circuitos.length ? (
            <p className="text-[12px] text-[#8a8a9c] text-center py-6">Sin circuitos. Agregá las cargas del tablero.</p>
          ) : (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        {(c.contactor || contactor) && (
                          <div className="mt-1 text-[10.5px] text-[#8a8a9c]">
                            Contactor: <span className="text-[#9a9aad]">{c.contactor || (contactor ? 'requiere (carga inductiva)' : '—')}</span>
                          </div>
                        )}
                        {c.notas && <div className="mt-1 text-[10.5px] text-[#8a8a9c] italic">{c.notas}</div>}
                      </div>
                      <div className="flex gap-1 flex-shrink-0">
                        <button onClick={() => onEditCircuito(c)} className="min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 p-1.5 text-[#8a8a9c] hover:text-[#a3e635]"><Pencil className="w-3.5 h-3.5" /></button>
                        <button onClick={() => onBorrarCircuito(c.id)} className="min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 p-1.5 text-[#8a8a9c] hover:text-[#ef4444]"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </div>
                  </div>
                )
...
function Stat({ label, valor }: { label: string; valor: string }) {
  return (
    <div className="bg-[#0d0d13] border border-[#1a1a24] rounded-lg px-2.5 py-2">
      <div className="text-[9.5px] text-[#8a8a9c] uppercase tracking-wide">{label}</div>
...
function UnifilarTab({ sel, circuitos, resumen }: { sel: Tablero | null; circuitos: Circuito[]; resumen: any }) {
  ...
  return (
    <div className="bg-[#101016] border border-[#1f1f2b] rounded-xl p-3 overflow-x-auto">
      ...
      <p className="text-[10px] text-[#8a8a9c] mt-2 px-1">
  ...
  return (
    <div className="space-y-4 max-w-[760px]">
      ...
      <Bloque titulo="Tablero de la bomba de ósmosis (diseño cerrado)">
        ...
        <p className="mt-2 text-[11px] text-[#8a8a9c]">Con 32A el interruptor maneja la bomba directo (no hace falta contactor). El botón manual del Gralf sirve si se cae el WiFi.</p>
      </Bloque>
      ...
      <Bloque titulo="Software para diseñar el tablero (gratis)">
        ...
        <p className="mt-2 text-[11px] text-[#8a8a9c]">No se integran por archivo: los datos pasan a mano. Flujo: dimensionar → simular → dibujar.</p>
      </Bloque>
    </div>
  )
}
