      ...
      <div className="sticky top-0 z-40 bg-[#0a0a0f]/95 backdrop-blur-[2px] border-b border-[#1f1f2b]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#8a8a9c]">G08 Personal · G10 Documental · 20 formularios CM-RE</div>
          </div>
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4">
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          ...
          <div className="bg-[#101016] border border-[#1f1f2b] rounded-xl p-3 space-y-1">
            <h3 className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium px-2 py-1">Codigos CUMCS</h3>
            {tipos.map(t => (
              <button key={t.codigo} onClick={() => { setCodigoSel(t.codigo); setValores({}) }}
                ...
                <p className="text-[11px] text-[#8a8a9c] mt-0.5 leading-snug">{t.nombre}</p>
              </button>
            ))}
          </div>
          ...
          <div className="lg:col-span-2 bg-[#101016] border border-[#1f1f2b] rounded-xl p-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {campos.map(c => (
                    <label key={c.key} className={c.tipo === 'textarea' ? 'sm:col-span-2 block' : 'block'}>
                      <span className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1.5 block">
                      ...
                      {c.tipo === 'textarea' ? (
                        <textarea value={valores[c.key] || ''} onChange={e => handleChange(c.key, e.target.value)} rows={3}
                          ...
                          className="w-full px-2.5 py-2 bg-[#15151d] border border-[#2a2a3a] rounded-lg text-xs text-[#d4d4dd] placeholder-[#8a8a9c] focus:outline-none focus:ring-1 focus:ring-[#a3e635]/30 focus:border-[#404d20]" />
                      ) : c.tipo === 'select' ? (
            ...
            ) : <p className="text-xs text-[#8a8a9c]">Elegi un codigo de la izquierda</p>}
          </div>
        </div>
        ...
        <div className="bg-[#101016] border border-[#1f1f2b] rounded-xl p-4">
          ...
          {cargandoReg ? <Loader2 className="w-5 h-5 animate-spin text-[#a3e635]" /> :
            registros.length === 0 ? <p className="text-xs text-[#8a8a9c]">Sin registros aun</p> : (
            <div className="space-y-0.5">
              {registros.map((r: any) => (
                <div key={r.id} className="flex items-center gap-3 py-2 border-b border-[#1f1f2b] last:border-0 text-xs">
                  ...
                  <span className="text-[#8a8a9c] tabular-nums">{r.fecha}</span>
