...
const coloresEstado: Record<string, { pill: string; dot: string }> = {
  ...
  consumido:  { pill: 'bg-[#5c5c6b]/12 border-[#1f1f2b] text-[#8a8a9c]',      dot: '#8a8a9c' },
}
...
function EstadoPill({ estado }: { estado: string }) {
  const cls = coloresEstado[estado]?.pill ?? 'bg-[#5c5c6b]/12 border-[#1f1f2b] text-[#8a8a9c]'
  return (
...
function SistemaPill({ sistema }: { sistema?: string }) {
  if (!sistema) return <span className="text-[11px] text-[#8a8a9c]">—</span>
...
export default function PaginaStock() {
  ...
  const columnas = useMemo<ColumnDef<any>[]>(() => [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2.5">
            <Icon className="w-4 h-4 text-[#8a8a9c] flex-shrink-0" strokeWidth={1.75} />
            <div>
              <p className="text-[12.5px] font-medium text-[#ececf1]">{item.productos?.nombre || '-'}</p>
              <p className="text-[10px] text-[#8a8a9c]">{ETIQUETAS_TIPO[item.productos?.tipo_producto] || item.productos?.tipo_producto}</p>
            </div>
          </div>
        )
      },
      ...
      cell: ({ row }) => (
        <div>
          ...
          {row.original.datos_extra?.camada && (
            <p className="text-[10px] text-[#8a8a9c]">Camada: {row.original.datos_extra.camada}</p>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-[12.5px] font-semibold text-[#ececf1] tabular-nums font-mono">
          ...
          <span className="font-normal text-[#8a8a9c] ml-1 text-[10px]">{unidadCorrecta(row.original.productos?.tipo_producto)}</span>
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      header: '',
      enableSorting: false,
      cell: () => <ChevronRight className="w-4 h-4 text-[#8a8a9c]" />,
    },
  ], [])
      ...
      <div className="flex-1 overflow-y-auto bg-[#0a0a0f] text-[#d4d4dd] font-sans ct-page-scroll">
        ...
        <div className="sticky top-0 z-40 bg-[#0a0a0f]/95 backdrop-blur-[2px] border-b border-[#1f1f2b]">
          <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
            ...
            {ultimaSinc && (
              <span className="hidden sm:inline text-[10px] text-[#8a8a9c] tabular-nums">
        ...
        <div className="px-3 sm:px-6 py-4 pb-20 space-y-3">
          ...
          <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] p-2.5 flex flex-wrap items-center gap-2 justify-between">
            ...
            <div className="hidden sm:flex items-center gap-1 bg-[#0a0a0f] border border-[#1f1f2b] rounded-lg p-0.5">
              <button onClick={() => setModo('tabla')}
                className={`p-1.5 rounded-md transition-colors ${modo === 'tabla' ? 'bg-[#15151d] text-[#d9f99d]' : 'text-[#8a8a9c] hover:text-[#d4d4dd]'}`}>
                <List className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => setModo('cards')}
                className={`p-1.5 rounded-md transition-colors ${modo === 'cards' ? 'bg-[#15151d] text-[#d9f99d]' : 'text-[#8a8a9c] hover:text-[#d4d4dd]'}`}>
          ...
          <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] overflow-hidden">
            <div className="flex flex-wrap gap-2 p-3">
              ...
              <div className="flex-1 min-w-[200px] relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8a8a9c]" />
                <input
                  type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)}
                  placeholder="Buscar código, producto, camada, traza…"
                  className="w-full pl-9 pr-4 py-2 bg-[#0a0a0f] border border-[#1f1f2b] focus:border-[#404d20] rounded-lg text-[12px] text-[#d4d4dd] placeholder-[#8a8a9c] focus:outline-none transition-colors"
                />
              </div>
              {/* Filtros toggle */}
              ...
              {filtrosActivos > 0 && (
                <button onClick={limpiarFiltros}
                  className="flex items-center gap-1 px-2 py-2 text-[11px] text-[#8a8a9c] hover:text-[#ff6b5a] transition-colors">
                  <X className="w-3.5 h-3.5" /> Limpiar
                </button>
              )}
              {/* Orden */}
              <div className="flex items-center gap-1 bg-[#0a0a0f] border border-[#1f1f2b] rounded-lg p-1 ml-auto">
                <span className="text-[10px] text-[#8a8a9c] px-1.5">Orden</span>
                {([
                  { v: 'fecha', label: 'Fecha' },
                  ...
                  <button key={o.v} onClick={() => { if (ordenCampo === o.v) setOrdenAsc(!ordenAsc); else setOrdenCampo(o.v) }}
                    className={`flex items-center gap-0.5 px-2 py-1 rounded-md text-[10.5px] font-medium transition-colors ${
                      ordenCampo === o.v ? 'bg-[#15151d] text-[#d9f99d]' : 'text-[#8a8a9c] hover:text-[#d4d4dd]'
                    }`}>
            ...
            {filtrosAbiertos && (
              <div className="border-t border-[#1f1f2b] p-4 bg-[#0a0a0f] space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] font-semibold text-[#8a8a9c] uppercase tracking-[0.12em] mb-2">Sistema</p>
                  ...
                  <div>
                    <p className="text-[10px] font-semibold text-[#8a8a9c] uppercase tracking-[0.12em] mb-2">Estado</p>
                ...
                <div>
                  <p className="text-[10px] font-semibold text-[#8a8a9c] uppercase tracking-[0.12em] mb-2">Tipo de producto</p>
                  <div className="flex flex-wrap gap-1.5">
                ...
                <div>
                  <p className="text-[10px] font-semibold text-[#8a8a9c] uppercase tracking-[0.12em] mb-2">Camada</p>
                  <div className="flex flex-wrap gap-1.5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={f.label}>
                      <p className="text-[10px] font-semibold text-[#8a8a9c] uppercase tracking-[0.12em] mb-1.5">{f.label}</p>
          ...
          {cargando ? (
            <div className="text-center py-14">
              ...
              <p className="mt-3 text-[12px] text-[#8a8a9c]">Cargando stock…</p>
            </div>
                ...
                return (
                  <div key={cam} className="rounded-xl bg-[#101016] border border-[#1f1f2b] overflow-hidden">
                    ...
                    <div className="px-4 py-3 bg-[#15151d] border-b border-[#1f1f2b] flex items-center justify-between gap-2">
                      ...
                      <span className="text-[10.5px] text-[#8a8a9c]">
                  ...
                  <>
                    <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] overflow-hidden">
                      <DataTable
                        ...
                        emptyState={
                          <div className="text-[#8a8a9c] py-6 text-center">
                    ...
                    {stockFiltrado.length > 0 && (
                      <div className="mt-2 flex items-center justify-between px-1 text-[11px] text-[#8a8a9c]">
                        ...
                        <button key={item.id} onClick={() => setSeleccionado(item)}
                          ...
                          <div className="flex items-start justify-between gap-2 mb-2">
                            <div className="flex items-center gap-2">
                              <Icon className="w-4 h-4 text-[#8a8a9c]" strokeWidth={1.75} />
                          ...
                          <div className="flex items-center justify-between mt-2">
                            <p className="text-[22px] font-bold font-mono tabular-nums text-[#ececf1] leading-none">
                              ...
                              <span className="text-[10px] text-[#8a8a9c] ml-1 font-normal">{unidadCorrecta(item.productos?.tipo_producto)}</span>
                            </p>
              ...
              <>
                ...
                <div className="flex items-center justify-between p-4 sm:p-5 border-b border-[#1f1f2b] sticky top-0 z-10 bg-[#101016]">
                  <div className="min-w-0">
                    ...
                    <p className="text-[11.5px] font-mono tabular-nums text-[#8a8a9c] truncate mt-0.5">{seleccionado.codigo_lote}</p>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => setSeleccionado(null)}
                      ...
                      <X className="w-5 h-5 text-[#8a8a9c]" />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
                  ...
                  {seleccionado.datos_extra?.sistema && (
                    <div className="p-3 rounded-xl bg-[#15151d] border border-[#1f1f2b]">
                      <p className="text-[10px] font-semibold text-[#8a8a9c] uppercase tracking-[0.12em] mb-2.5">Sistema de producción</p>
                  ...
                  {seleccionado.datos_extra && Object.keys(seleccionado.datos_extra).length > 0 && (
                    <div className="p-3 rounded-xl bg-[#15151d] border border-[#1f1f2b]">
                      <p className="text-[10px] font-semibold text-[#8a8a9c] uppercase tracking-[0.12em] mb-2.5">Datos adicionales</p>
                      <div className="grid grid-cols-2 gap-2.5">
                        {Object.entries(seleccionado.datos_extra).map(([key, value]) => (
                          <div key={key}>
                            <p className="text-[10px] text-[#8a8a9c]">{key.replace(/_/g, ' ')}</p>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <Clock className="w-3.5 h-3.5 text-[#8a8a9c]" />
                      <p className="text-[10px] font-semibold text-[#8a8a9c] uppercase tracking-[0.12em]">Historial de movimientos</p>
                    </div>
                    {cargandoMov ? (
                      <div className="flex items-center gap-2 text-[12px] text-[#8a8a9c] py-6 justify-center">
                      ...
                      <div className="text-center py-6 rounded-xl bg-[#15151d] border border-[#1f1f2b]">
                        <p className="text-[12px] text-[#8a8a9c]">Sin operaciones registradas</p>
                      </div>
                    ) : (
                          ...
                          return (
                            <div key={mov.id} className="flex gap-3">
                              ...
                              <div className="flex-1 pb-3">
                                ...
                                <div className="flex flex-wrap gap-x-3 text-[10.5px] text-[#8a8a9c] mt-0.5 font-mono tabular-nums">
                                ...
                                {mov.observaciones && <p className="text-[10.5px] text-[#8a8a9c] mt-0.5 truncate">{mov.observaciones}</p>}
                              </div>
                            </div>
                          )
                        })}
  ...
  return (
    <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] p-3.5 hover:border-[#404d20] transition-colors">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium leading-tight">{label}</p>
  ...
  return (
    <div onClick={onClick} className="flex items-center gap-3 px-4 py-2.5 hover:bg-[#15151d] cursor-pointer transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-[#8a8a9c]">{ETIQUETAS_TIPO[item.productos?.tipo_producto]} · {item.instalaciones?.nombre || 'sin ubicación'}</p>
      </div>
      ...
      <EstadoPill estado={item.estado} />
      <ChevronRight className="w-4 h-4 text-[#8a8a9c]" />
    </div>
  )
}
  ...
  return (
    <button onClick={onClick}
      ...
      <div className="flex items-center gap-2 mb-2.5">
        <Icon className="w-3.5 h-3.5 text-[#8a8a9c] flex-shrink-0" strokeWidth={1.75} />
      ...
      <div className="flex items-end justify-between gap-2">
        <span className="font-display font-bold text-[26px] text-[#ececf1] leading-none tabular-nums font-mono">
          {parseFloat(item.cantidad).toLocaleString('es-AR')}
          <span className="text-[11px] text-[#8a8a9c] ml-1 font-normal">{unidadCorrecta(item.productos?.tipo_producto)}</span>
        </span>
        <div className="text-right text-[10px] text-[#8a8a9c] leading-tight">
...
function InfoDark({ label, valor }: { label: string; valor?: string | null }) {
  return (
    <div>
      <p className="text-[10px] text-[#8a8a9c] uppercase tracking-[0.10em]">{label}</p>
