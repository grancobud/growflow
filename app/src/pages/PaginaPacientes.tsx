 ...
 */
export default function PaginaPacientes({ embebida = false }: { embebida?: boolean } = {}) {
        ...
        <div className={`flex items-center flex-wrap gap-2 sm:gap-x-4 ${embebida ? '' : 'px-3 sm:px-6 py-3'}`}>
          <div className="min-w-0">
            ...
            <div className={embebida
              ...
              : 'mt-0.5 text-[10.5px] sm:text-[11px] text-[#8a8a9c]'}>
          ...
          <div className="relative">
            ...
            <input value={busqueda} onChange={e => setBusqueda(e.target.value)}
              ...
              className="pl-8 pr-3 py-2 rounded-lg bg-[#15151d] border border-[#2a2a3a] text-[16px] sm:text-[12px] text-[#ececf1] placeholder-[#8a8a9c] focus:outline-none focus:border-[#a3e635]/60 w-[180px] min-h-[44px] sm:min-h-0" />
          </div>
      ...
      <div className={embebida ? '' : 'px-3 sm:px-6 py-4 pb-20'}>
        ...
        {cargando ? (
          ...
          <div className="py-16 text-center">
            ...
            <div className="mt-1 text-[11.5px] text-[#8a8a9c]">
                ...
                <div key={p.id} className="rounded-xl bg-[#101016] border border-[#1f1f2b] hover:border-[#404d20] transition-colors overflow-hidden">
                  <div className="p-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      ...
                      <button onClick={() => borrar(p)} className="min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 p-1.5 text-[#8a8a9c] hover:text-[#ff8a7a] hover:bg-[#15151d] rounded-lg transition-colors ml-auto" title="Borrar ficha">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative w-full ${ancho} max-h-[90vh] overflow-y-auto rounded-xl bg-[#101016] border border-[#2a2a3a] shadow-2xl`}>
        <div className="sticky top-0 bg-[#101016] flex items-center justify-between px-5 py-3.5 border-b border-[#1f1f2b]">
          ...
          <button onClick={onCerrar} className="min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 p-1 text-[#8a8a9c] hover:text-[#ececf1]" aria-label="Cerrar"><X className="w-4 h-4" /></button>
        </div>
  ...
  return (
    <Modal titulo={p.nombre_completo} ancho="max-w-2xl" onCerrar={onCerrar}>
      ...
      <div className="mt-4 pt-4 border-t border-[#1f1f2b]">
        <div className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] mb-2">Credencial REPROCANN</div>
        {p.credencial_url ? (
          cred === 'error' ? (
            <p className="text-[11.5px] text-[#f0a5a5]">No se pudo abrir la credencial. Recargá la página o volvé a subir el archivo.</p>
          ) : !cred ? (
            <p className="text-[11.5px] text-[#8a8a9c]">Abriendo la credencial…</p>
          ) : (
            <div>
          ...
          <p className="text-[11.5px] text-[#8a8a9c]">Sin credencial cargada. Editá la ficha para subir el PDF.</p>
        )}
      </div>

      {/* Plantas asignadas */}
      <div className="mt-4 pt-4 border-t border-[#1f1f2b]">
        <div className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] mb-2">Plantas en cultivo para este paciente ({plantasPac.length})</div>
        {plantasPac.length === 0 ? (
          <p className="text-[11.5px] text-[#8a8a9c]">Todavía no hay plantas asignadas. Asigná el paciente desde la planta (Plantas → ver ficha).</p>
        ) : (
