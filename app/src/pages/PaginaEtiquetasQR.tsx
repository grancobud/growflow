      ...
      <div className="sticky top-0 z-40 bg-[#0a0a0f]/95 backdrop-blur-[2px] border-b border-[#1f1f2b]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#8a8a9c]">{lotes.length} lotes con código QR público para escanear</div>
          </div>
        ...
        <div className="flex flex-wrap gap-3 bg-[#101016] border border-[#1f1f2b] rounded-xl p-3">
          <div className="flex-1 min-w-[200px] relative">
            ...
            <input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)}
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-[#0a0a0f] border border-[#1f1f2b] rounded-lg text-[13px] text-[#d4d4dd] placeholder-[#8a8a9c] focus:outline-none focus:border-[#404d20] transition-colors" />
          </div>
                ...
                <div key={l.id} className="bg-[#101016] border border-[#1f1f2b] hover:border-[#404d20] rounded-xl p-4 space-y-3 transition-colors">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <p className="text-[10px] uppercase text-[#8a8a9c] font-semibold tracking-wide">{l.productos?.tipo_producto?.replace(/_/g, ' ')}</p>
        ...
        <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4" onClick={() => setVerQR(null)}>
          <div className="bg-[#101016] border border-[#1f1f2b] rounded-2xl shadow-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="text-[10px] uppercase font-semibold text-[#8a8a9c] tracking-wide">Código QR público</p>
              ...
              <button onClick={() => setVerQR(null)} className="text-[#8a8a9c] hover:text-[#d4d4dd] transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            {/* QR sobre fondo blanco — necesario para escaneo */}
            ...
            <p className="text-[11px] text-center text-[#8a8a9c] mt-3 font-mono break-all">{urlDe(verQR.codigo_lote)}</p>
