  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#0a0a0f] text-[#d4d4dd] font-sans ct-page-scroll">
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        <Link
          to="/trazabilidad"
          className="inline-flex items-center gap-2 text-[11.5px] text-[#8a8a9c] hover:text-[#d9f99d] font-medium transition-colors"
        >
          ...
          >
            ...
            >
              <div className="flex items-start gap-4 flex-wrap">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium">Lote</p>
            ...
            {datos.operaciones.length > 0 && (
              ...
              >
                <h3 className="text-[13px] font-display font-bold text-[#ececf1] mb-3 flex items-center gap-2">
                  ...
                  Operaciones seed-to-sale
                  <span className="text-[11px] font-normal text-[#8a8a9c]">({datos.operaciones.length})</span>
                </h3>
                <ul className="divide-y divide-[#1f1f2b]">
                  {datos.operaciones.map((o) => (
                    <li key={o.id} className="flex items-center gap-3 py-2 text-[11.5px]">
                      ...
                      {o.responsable && <span className="text-[#8a8a9c] truncate hidden sm:inline">· {o.responsable}</span>}
            ...
            >
              <h3 className="text-[13px] font-display font-bold text-[#ececf1] mb-3 flex items-center gap-2">
                ...
                <span className="text-[11px] font-normal text-[#8a8a9c]">({datos.registros.length})</span>
              </h3>
                  ...
                  {datos.registros.map((r) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="text-[#8a8a9c] truncate flex-1 text-left hidden md:inline">{r.usuario_email}</span>
                      ...
                      {expandido === r.id && (
                        ...
                        >
                          <div>
                            <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1">Hash SHA-256</p>
                            <p className="font-mono text-[10px] text-[#bef264] break-all">{r.hash_sha256}</p>
                          </div>
                          <div>
                            <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1">Hash anterior (encadenado)</p>
                            <p className="font-mono text-[10px] text-[#8a8a9c] break-all">{r.hash_anterior || '(primer registro de la cadena)'}</p>
                          </div>
                          {r.campos_modificados && Array.isArray(r.campos_modificados) && r.campos_modificados.length > 0 && (
                            <div>
                              <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1">Campos modificados</p>
                          ...
                          {r.datos_nuevos && (
                            <details>
                              <summary className="cursor-pointer text-[#8a8a9c] hover:text-[#b3b3c0] font-medium text-[10.5px]">
                                Ver JSON completo
                              </summary>
                          ...
                          {r.ip_origen && (
                            <p className="text-[#8a8a9c] flex items-center gap-1 text-[10.5px]">
...
function InfoMini({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium">{label}</p>
