      ...
      <div className="sticky top-0 z-40 bg-[#0a0a0f]/95 backdrop-blur-[2px] border-b border-[#1f1f2b]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#8a8a9c]">Análisis visual y comparativo por camada</div>
          </div>
        ...
        <div className="flex items-center gap-1.5 px-3 sm:px-6 py-2 flex-wrap">
          ...
          {tab !== 'kpis' && tab !== 'heatmap' && (
            <div className="flex items-center gap-1.5 ml-2 flex-wrap">
              <span className="text-[10px] uppercase text-[#8a8a9c] font-semibold tracking-wide">Camada:</span>
              {CAMADAS.map(c => (
      ...
      <div className="ct-dash-wrap p-4 sm:p-6 space-y-4">
                  ...
                  return (
                    <motion.div key={c.camada}
                      ...
                      <div className="flex items-center justify-between mb-3 gap-2">
                        ...
                        <div className="text-[11px] text-[#8a8a9c] font-mono tabular-nums shrink-0">
        ...
        {tab === 'timeline' && (
          <Suspense fallback={<TabLoader />}>
            {cargando ? <TabLoader /> :
             timeline.length === 0
               ? <p className="text-center py-12 text-[#8a8a9c]">Sin eventos para {camadaSel}</p>
...
function KpiCard({ label, valor, sub, tone }: { label: string; valor: any; sub: string; tone: 'primary' | 'gold' | 'warn' }) {
  ...
  return (
    <div className={`bg-[#101016] border border-[#1f1f2b] border-l-[3px] ${accent} rounded-xl p-4`}>
      <p className="text-[10px] uppercase tracking-wide font-semibold text-[#8a8a9c] mb-1">{label}</p>
...
function MiniStat({ label, val }: { label: string; val: any }) {
  return (
    <div className="bg-[#15151d] border border-[#1f1f2b] rounded-lg p-2">
      <p className="text-[9px] uppercase text-[#8a8a9c] font-semibold tracking-wide">{label}</p>
  ...
  return (
    <div>
      <div className="flex justify-between items-baseline mb-1">
        ...
        <span className="text-[10.5px] text-[#8a8a9c] font-mono tabular-nums">{left} → {right}</span>
      </div>
