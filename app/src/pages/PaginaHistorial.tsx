      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="flex-1 min-w-[180px] relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#8a8a9c]" strokeWidth={1.8} />
            <input
              type="text"
              value={busqueda}
              onChange={(e) => setBusqueda(e.target.value)}
              placeholder="Buscar lote, responsable…"
              className="w-full pl-9 pr-4 py-2 bg-[#101016] border border-[#1f1f2b] hover:border-[#2a2a3a] focus:border-[#404d20] rounded-lg text-[12px] text-[#d4d4dd] placeholder-[#8a8a9c] outline-none transition-colors"
            />
          </div>
                ...
                <thead>
                  <tr className="border-b border-[#1f1f2b]">
                    {['Operacion','Lote','Cantidad','Estado','Fecha','Operador'].map(h => (
                      <th key={h} className="px-4 py-2.5 text-left text-[10px] uppercase tracking-[0.12em] text-[#8a8a9c] font-medium">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  ...
                  <div key={op.id} className="rounded-xl bg-[#101016] border border-[#1f1f2b] p-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10.5px] text-[#8a8a9c] font-mono tabular-nums mt-0.5">
                          {lote} · {cantidad}
                        </p>
                        {op.observaciones && (
                          <p className="text-[10.5px] text-[#8a8a9c] mt-0.5 truncate">{op.observaciones}</p>
                        )}
                        <p className="text-[10px] text-[#8a8a9c] mt-1 font-mono tabular-nums">
        ...
        {/* Footer */}
        <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] px-4 py-2.5 text-[10px] text-[#8a8a9c] text-center">
