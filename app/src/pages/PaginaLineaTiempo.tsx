    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0a0f] text-[#d4d4dd] font-sans">
      <div className="sticky top-0 z-40 bg-[#0a0a0f]/95 backdrop-blur-[2px] border-b border-[#1f1f2b]">
        <div className="flex items-center flex-wrap gap-2 px-3 sm:px-6 py-3">
          ...
          <span className="text-[10.5px] sm:text-[11px] text-[#8a8a9c]">{perfiles.length} variedades</span>
        </div>
      ...
      {cargando ? (
        ...
        <div className="py-16 text-center">
          ...
          <div className="mt-1 text-[11.5px] text-[#8a8a9c]">Cargá plantas con fecha de germinación para ver su línea de tiempo.</div>
        </div>
      ) : (
        <div className="px-3 sm:px-6 py-4 pb-24 space-y-2.5">
            ...
            return (
              <div key={p.gen.id} className="rounded-xl bg-[#101016] border border-[#1f1f2b] hover:border-[#404d20] transition-colors p-3 sm:p-3.5">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  <button onClick={() => setEdit(p)} className="min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 p-1 text-[#8a8a9c] hover:text-[#d9f99d] hover:bg-[#15151d] rounded-lg transition-colors" title="Editar / ver ficha"><Pencil className="w-3.5 h-3.5" /></button>
                </div>
                ...
                <div className="mt-1.5 flex flex-wrap gap-x-4 gap-y-0.5 text-[10.5px] text-[#8a8a9c]">
          ...
          <p className="text-[10.5px] text-[#8a8a9c] pt-2">En automáticas el tiempo de la ficha es el ciclo total (semilla a cosecha). En feminizadas el vegetativo es indefinido; los días de floración corren desde que las pasás a 12/12. La línea punteada marca hoy.</p>
        </div>
      )}
      ...
      <div className="bg-[#0d0d12] border border-[#1f1f2b] w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-[#0d0d12] border-b border-[#1f1f2b] px-4 py-3 flex items-center justify-between">
          <div className="min-w-0">
            ...
            <div className="text-[10.5px] text-[#8a8a9c]">{perfil.auto ? 'Automática' : 'Feminizada'}{g.genotipo ? ' · ' + g.genotipo : ''}{g.banco ? ' · ' + g.banco : ''}</div>
          </div>
        ...
        <div className="p-4 space-y-3">
          <div><label className={labelCls}>Fecha de germinación</label><input type="date" className={inputCls} value={germ} onChange={e => setGerm(e.target.value)} /><p className="text-[10px] text-[#8a8a9c] mt-1">Se aplica a las {perfil.cantidad} plantas de esta variedad.</p></div>
