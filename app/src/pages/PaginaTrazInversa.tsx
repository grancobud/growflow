        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8a8a9c] pointer-events-none" />
              <input
                type="text"
                value={q}
                onChange={(e) => setQ(e.target.value)}
                placeholder="25.FIS.BI1.C7.1  ·  ALM-C9-COCO  ·  25.Tr8.Ka.PM4.C11"
                className="w-full pl-9 pr-3 py-2.5 bg-[#0a0a0f] border border-[#1f1f2b] focus:border-[#404d20] rounded-md text-[12.5px] text-[#ececf1] placeholder:text-[#8a8a9c] font-mono outline-none transition-colors"
              />
            </div>
            <button
              type="submit"
        ...
        <AnimatePresence mode="wait">
          ...
          {!traza && !cargando && (
            ...
            >
              ...
              <p className="text-[12.5px] text-[#8a8a9c]">Ingresa un codigo de lote para ver la cadena inversa</p>
              <p className="text-[11px] text-[#8a8a9c] mt-1">Ej: 25.PM4.CL11.Li2.VG001.SF2</p>
            </motion.div>
          )}
            ...
            >
              ...
              <div className="rounded-xl bg-[#a3e635]/8 border border-[#404d20] p-5 flex items-center gap-4 flex-wrap">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium">Cadena encontrada</p>
                  <p className="font-mono font-bold text-[#ececf1] text-[16px] break-all">{traza.codigo_buscado}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium">Camada</p>
              ...
              <section className="rounded-xl bg-[#101016] border border-[#1f1f2b] p-5">
                <h3 className="text-[13px] font-display font-bold text-[#ececf1] mb-4 flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-normal text-[#8a8a9c]">{traza.cadena.length} etapas</span>
                </h3>
                <motion.ol
                  className="space-y-0"
              ...
              {traza.analisis_asociados?.length > 0 && (
                <section className="rounded-xl bg-[#101016] border border-[#1f1f2b] p-5">
                  <h3 className="text-[13px] font-display font-bold text-[#ececf1] mb-3 flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-normal text-[#8a8a9c]">({traza.analisis_asociados.length})</span>
                  </h3>
                  <ul className="divide-y divide-[#1f1f2b]">
                    {traza.analisis_asociados.map((a: any, i: number) => (
                      <li key={i} className="flex items-center gap-3 py-2 text-[11.5px] flex-wrap">
                        ...
                        <span className="text-[#8a8a9c] tabular-nums">{a.fecha_analisis}</span>
