      ...
      <div className="no-print px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] p-4">
          <div className="flex flex-wrap items-end gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1.5">Año</p>
              <div className="relative">
                ...
                <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#8a8a9c] pointer-events-none" />
              </div>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1.5">Semestre</p>
              <div className="relative">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {[
                  ...
                  <div key={label} className="rounded-lg bg-[#15151d] border border-[#1f1f2b] p-2.5">
                    <p className="text-[9px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium">{label}</p>
        ...
        <div className="rounded-xl bg-[#101016] border border-[#1f1f2b] p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-semibold text-[13px] text-[#ececf1]">
                Documentos adjuntos <span className="text-[#8a8a9c] tabular-nums">({adjuntos.length})</span>
              </h3>
            </div>
          ...
          <p className="text-[10px] text-[#8a8a9c] mb-2">Imagenes, PDFs, Excel, Word, ZIP. Signed URLs 1h.</p>
          {adjuntos.length === 0 ? (
            <p className="text-[11.5px] text-[#8a8a9c] text-center py-4">Sin archivos todavia</p>
          ) : (
            <ul className="divide-y divide-[#1f1f2b]">
              {adjuntos.map(a => (
                <li key={a.name} className="flex items-center gap-3 py-2 text-[11.5px]">
                  <Paperclip className="w-3 h-3 text-[#8a8a9c] flex-shrink-0" />
                  <a href={a.url} target="_blank" rel="noopener noreferrer"
                    className="font-mono text-[#bef264] hover:text-[#d9f99d] truncate flex-1">
                    {a.name.replace(/^\d+_/, '')}
                  </a>
                  <span className="text-[#8a8a9c] tabular-nums">{(a.size / 1024).toFixed(1)} KB</span>
                  <span className="text-[#8a8a9c] hidden sm:block tabular-nums">
                    {a.created_at ? new Date(a.created_at).toLocaleDateString('es-AR') : ''}
                  </span>
                  <button onClick={() => eliminarAdjunto(a.name)}
                    className="text-[#8a8a9c] hover:text-[#ff8a7a] flex-shrink-0">
              ...
              {historial.map(h => {
                ...
                return (
                  <li key={h.id} className="flex items-center justify-between gap-3 px-4 py-2.5 text-[11.5px] hover:bg-[#15151d] transition-colors">
                    ...
                    <span className="text-[#8a8a9c] tabular-nums hidden sm:block">
                    ...
                    <span className="text-[#8a8a9c] font-mono">{h.numero_presentacion || '—'}</span>
                  </li>
                )
              })}
            </ul>
...
function FormOrg({ org, onCancel, onGuardar, guardando }: { org: any; onCancel: () => void; onGuardar: (d: any) => void; guardando: boolean }) {
  ...
  const input = (k: string, label: string, placeholder?: string) => (
    <label key={k} className="block">
      <span className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-1 block">{label}</span>
      <input type="text" value={f[k] || ''} onChange={e => setF({...f, [k]: e.target.value})}
        placeholder={placeholder}
        className="w-full px-3 py-2 bg-[#15151d] border border-[#1f1f2b] hover:border-[#2a2a3a] focus:border-[#404d20] rounded-lg text-[12px] text-[#d4d4dd] placeholder-[#8a8a9c] outline-none transition-colors" />
    </label>
  )
  return (
