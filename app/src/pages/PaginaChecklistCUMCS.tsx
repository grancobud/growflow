    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0a0f] text-[#d4d4dd] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0a0f]/95 backdrop-blur-[2px] border-b border-[#1f1f2b]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#8a8a9c]">{totalCompletados}/{totalReqs} con datos ({porcentaje}%) · <span className="hidden sm:inline">cruzado con Excel CM-RE-1010</span></div>
          </div>
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        <div className="bg-[#101016] rounded-xl border border-[#1f1f2b] p-4 sm:p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex-1 min-w-0">
              ...
              {ultimaSinc && (
                <p className="text-[10px] text-[#8a8a9c] mt-0.5">Ultima sincronizacion: {ultimaSinc.toLocaleTimeString()}</p>
              )}
            </div>
          ...
          <div className="flex flex-wrap gap-4 mt-3 text-[11px] text-[#8a8a9c]">
        ...
        <div className="bg-[#101016] rounded-xl border border-[#1f1f2b] p-4">
          <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-3">Camadas rastreadas</p>
        ...
        <div className="flex gap-2 sm:gap-3">
          <div className="flex-1 relative">
            ...
            <input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)}
              ...
              className="w-full pl-9 pr-4 py-2 bg-[#101016] border border-[#1f1f2b] rounded-lg text-[13px] text-[#d4d4dd] placeholder-[#8a8a9c] focus:outline-none focus:ring-1 focus:ring-[#a3e635]/30 focus:border-[#404d20] transition-colors" />
          </div>
              ...
              return (
                <div key={area.id} className="rounded-xl border border-[#1f1f2b] bg-[#101016] overflow-hidden">
                  <button onClick={() => setAreaAbierta(isAreaOpen && !busqueda ? null : area.id)}
                    ...
                    {isAreaOpen ? <ChevronDown className="w-4 h-4 text-[#8a8a9c]" /> : <ChevronRight className="w-4 h-4 text-[#8a8a9c]" />}
                    ...
                    <div className="flex-1 text-left min-w-0">
                      ...
                      <p className="text-[10px] text-[#8a8a9c] hidden sm:block">{area.descripcion}</p>
                    </div>
              ...
              return (
                <div key={grupo.grupo} className="bg-[#101016] rounded-xl border border-[#2a2a3a] overflow-hidden">
                  <button onClick={() => setGrupoAbierto(isOpen && !busqueda ? null : grupo.grupo)}
                    ...
                    {isOpen ? <ChevronDown className="w-4 h-4 text-[#8a8a9c]" /> : <ChevronRight className="w-4 h-4 text-[#8a8a9c]" />}
                    <div className="flex-1 text-left min-w-0">
                    ...
                    return (
                      <div key={sub.nombre} className="border-t border-[#1f1f2b]">
                        <button onClick={() => setSubAbierta(subOpen && !busqueda ? null : subKey)}
                          ...
                          <span className="text-[10px] font-semibold text-[#8a8a9c] bg-[#15151d] border border-[#2a2a3a] px-2 py-0.5 rounded-full">{subComp}/{subTotal}</span>
                        </button>
                        {subOpen && (
                            ...
                            <div className="flex items-center gap-3 px-4 py-3 pl-10 hover:bg-[#15151d] cursor-pointer transition-colors"
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 flex-wrap">
                                  {mostrarCodigos && <span className="text-[11px] font-mono text-[#8a8a9c] opacity-60">{reg.codigo}</span>}
                                ...
                                <p className={`text-sm ${tieneData ? 'text-[#ececf1]' : 'text-[#8a8a9c]'}`}>{reg.nombre}</p>
                              </div>
                              {isExpanded ? <ChevronDown className="w-3.5 h-3.5 text-[#8a8a9c]" /> : <ChevronRight className="w-3.5 h-3.5 text-[#8a8a9c]" />}
                            </div>

                            {isExpanded && (
                              <div className="px-4 py-3 pl-16 bg-[#0a0a0f]/50 border-t border-[#1f1f2b]">
                                {reg.procedimiento_referencia && (
                                  <p className="text-[11px] text-[#8a8a9c] mb-2">Procedimiento: {reg.procedimiento_referencia}</p>
                                )}
                                <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium mb-2">Estado por camada</p>
                                    ...
                                    return (
                                      <div key={cam}
                                        ...
                                        {estadoFinal === 'registrado' ? (
                                          ...
                                          <span className="text-[10px] text-[#8a8a9c]">—</span>
                                        )}
                                        ...
                                        <span className={`text-[10px] ml-auto font-medium flex items-center gap-1 ${
                                          ...
                                          estadoFinal === 'pendiente' ? 'text-[#c4b5fd]' : 'text-[#8a8a9c]'
                                        }`}>
        ...
        <div className="p-3 bg-[#101016] border border-[#1f1f2b] rounded-xl text-[11px] text-[#8a8a9c] text-center">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4" onClick={() => setVerRegistro(null)}>
          <div className="bg-[#101016] border border-[#1f1f2b] rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-[#1f1f2b] flex items-start justify-between gap-3">
              ...
              <button onClick={() => setVerRegistro(null)} className="p-1.5 rounded-lg hover:bg-[#1c1c27] text-[#8a8a9c] hover:text-[#d4d4dd] flex-shrink-0">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4">
              {cargandoRegistros ? (
                <div className="text-center py-8">
                  ...
                  <p className="mt-3 text-sm text-[#8a8a9c]">Buscando registros para {verRegistro.camada}...</p>
                </div>
                  ...
                  return (
                    <div key={r.id || idx} className="border border-[#2a2a3a] bg-[#15151d] rounded-xl overflow-hidden">
                      <div className="px-4 py-2 border-b border-[#1f1f2b] flex items-center justify-between">
                        <span className="text-[11px] font-mono text-[#8a8a9c]">#{idx + 1}</span>
                        <span className="text-[10px] text-[#8a8a9c]">id: {String(r.id).slice(0, 8)}...</span>
                      </div>
                      <div className="p-4 grid grid-cols-2 gap-x-4 gap-y-2">
                        {campos.map(([k, v]) => (
                          <div key={k}>
                            <p className="text-[10px] uppercase tracking-[0.14em] text-[#8a8a9c] font-medium">{k}</p>
