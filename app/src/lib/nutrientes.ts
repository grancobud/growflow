...
export interface Sal {
  ...
  docs?: { nombre: string; archivo: string }[]  // fichas/PDFs en public/docs/
  custom?: boolean
}
...
export const SALES_DEFECTO: Sal[] = [
  ...
  { id: 'ryano_mikro', nombre: 'Ryanodine Mikro (B)', bidon: 'C', liquido: true, densidad: 1.1,
    // Verificado contra la etiqueta del envase: Mg 1.00, S 1.35, Fe 0.109,
    // Mn 0.020, Zn 0.005, B 0.008, Cu 0.003, Mo 0.0005.
    // OJO: el PDF "Ryanodine ABC — Información del Producto" dice Zn 0.004 en vez
    // de 0.005; el resto de los valores coincide. Se usa el de la etiqueta, que es
    // lo que viene impreso en el bidón. La diferencia son 0,044 ppm a 4 mL/L.
  ...
  // Etiqueta PRO-CORE-25B rev. A01.008. Ca 17% (estaba estimado en 16) y micros
  // del guaranteed analysis (estaban hasta 5 veces por encima).
  // El N va repartido, aunque la etiqueta lo declare 14% todo nítrico: la fuente
  // es nitrato de calcio-amonio, que trae ~7% de su N como amonio. Con Ca 17% son
  // 89,5% de CAN → NO3-N 12,88% + NH4-N 0,98% = 13,87%, contra el 14% declarado.
  ...
  { id: 'athena_cleanse', nombre: 'Athena Cleanse (limpieza)', formula: 'HOCl', bidon: 'C', aditivo: true, liquido: true, densidad: 1.0, comp: {},
    descripcion: 'Comercial Athena: NO es nutriente. Ácido hipocloroso (HOCl) al 0,028%, el resto (99,972%) es inerte. Limpia raíces y sistema de riego, y previene la acumulación mineral y el biofilm. pH neutro, no tóxico, se puede usar en todas las etapas. Dosis de la etiqueta: mantenimiento 5-13 ml por 10 L (0,5-1,3 ml/L); condiciones severas 13-26 ml por 10 L (1,3-2,6 ml/L). Vida útil 2 años, guardar al reparo de la luz.' },
  { id: 'hocl', nombre: 'Ácido hipocloroso (generador en polvo)', formula: 'HOCl', bidon: 'C', aditivo: true, comp: {},
    descripcion: 'Materia prima genérica para clonar Athena Cleanse. Generador de HOCl a base de sal, en POLVO: estable +2 años (el HOCl líquido se degrada rápido). Sanitizante de amplio espectro (bacterias, hongos, algas, biofilm), pH neutro, biodegradable, no agresivo. NO es nutriente. Ej. comercial: Binal BioMax (Axel Química). DATO: Athena Cleanse = 0,028% HOCl = 280 ppm (SDS). Binal ficha: 0,7 g/L = 100 ppm cloro activo. RECETA (Gastón): stock = 10 g de Binal en 5 L de agua (0,28 g/L ≈ 280 ppm, iguala al Cleanse); dosis = 1 mL de stock por litro de riego (misma dosis que Cleanse a 1 mL/L). Si querés igualar Cl2-equivalente exacto: 13 g/5 L. Rinde: 10 g → 5 L stock → dosifica a 1 mL/L; un balde de 6 kg ≈ 3 millones de L de riego.',
    docs: [
      { nombre: 'Binal BioMax — ficha técnica (Axel Química)', archivo: 'binal-biomax-ficha-tecnica.pdf' },
      { nombre: 'Athena Cleanse — SDS (0,028% HOCl)', archivo: 'athena-cleanse-sds.pdf' },
    ] },
  ...
  { id: 'aa_glicina', nombre: 'L-Glicina (polvo)', formula: 'C₂H₅NO₂', bidon: 'C', aditivo: true, comp: {},
    descripcion: '⭐ Quelante natural de micros + forma clorofila. El aa más chico (entra fácil a la hoja). Dosis foliar ~10–30 mg/L.' },
  { id: 'aa_glutamico', nombre: 'L-Ácido glutámico (polvo)', formula: 'C₅H₉NO₄', bidon: 'C', aditivo: true, comp: {},
    descripcion: 'Base de la asimilación de nitrógeno + síntesis de clorofila. Mantiene fotosíntesis bajo estrés. Dosis foliar ~10–30 mg/L.' },
  { id: 'aa_prolina', nombre: 'L-Prolina (polvo)', formula: 'C₅H₉NO₂', bidon: 'C', aditivo: true, comp: {},
    descripcion: '🛡️ Osmoprotector anti-estrés (calor, sequía, frío): estabiliza membranas y pared celular. La planta la acumula al sufrir. Dosis foliar ~10–30 mg/L.' },
  { id: 'vit_b1', nombre: 'Vitamina B1 (tiamina, polvo)', formula: 'C₁₂H₁₇N₄OS', bidon: 'C', aditivo: true, comp: {},
    descripcion: 'Antioxidante que ayuda contra estrés (salinidad, patógenos). OJO: el "mito B1 = más raíces" NO tiene respaldo fuerte; su valor real es anti-estrés. Dosis ínfima ~1–5 mg/L (por stock). Farmacia/Pura Química.' },
  { id: 'humico_fulvico', nombre: 'Ácido húmico/fúlvico (leonardita, polvo)', formula: 'leonardita', bidon: 'C', aditivo: true, comp: {},
...
export function kitParaPerfil(p: Perfil, opts: OpcionesKit = {}): string[] {
  ...
  // --- Potasio ---
  if (has('K')) {
    if (has('NO3')) kit.add('kno3') // K + nítrico SOLO si el perfil pide nítrico (no contamina perfiles amoniacales)
