...
describe('normalizarProveedor', () => {
  it('saca los espacios de sobra', () => {
    expect(normalizarProveedor('  Pura   Química  ')).toBe('Pura Química')
  })
  ...
  it('NO cambia mayúsculas', () => {
    ...
    // «Pura Química» en «pura química» en el recibo.
    expect(normalizarProveedor('PINITO')).toBe('PINITO')
  })
})
...
describe('listaDeProveedores', () => {
  it('junta las fuentes y no repite', () => {
    ...
    expect(listaDeProveedores([doc('Pinito')], [doc('Pura Química'), doc('Pinito')]))
      .toEqual(['Pinito', 'Pura Química'])
  })
  ...
  it('ignora vacíos, nulos y espacios', () => {
    expect(listaDeProveedores([doc(null), doc(''), doc('   '), doc('Pinito')]))
      .toEqual(['Pinito'])
  })

  it('con dos grafías ofrece LA MÁS USADA', () => {
    // El caso real de Panacea: `Cristian` 149 veces y `CRISTIAN` un par. Ofrecer
    // la rara empujaría a seguir partiendo los totales.
    const lista = listaDeProveedores([doc('Pinito'), doc('Pinito'), doc('PINITO')])
    expect(lista).toEqual(['Pinito'])
  })
  ...
  it('a igual uso da lo mismo, venga en el orden que venga', () => {
    ...
    const a = listaDeProveedores([doc('PINITO'), doc('Pinito')])
    const b = listaDeProveedores([doc('Pinito'), doc('PINITO')])
    expect(a).toEqual(b)
    expect(a).toHaveLength(1)
  })

  it('trata los espacios de más como el mismo proveedor', () => {
    expect(listaDeProveedores([doc('Pura Química'), doc('Pura  Química')]))
      .toEqual(['Pura Química'])
  })
...
describe('proveedorYaExiste', () => {
  const lista = ['Pinito', 'Pura Química']

  it('encuentra el que ya está, escrito de otra forma', () => {
    expect(proveedorYaExiste('PINITO', lista)).toBe('Pinito')
    expect(proveedorYaExiste('  pinito ', lista)).toBe('Pinito')
  })

  it('devuelve undefined si es realmente nuevo', () => {
    expect(proveedorYaExiste('Jardinero Correntino', lista)).toBeUndefined()
  })
...
// Panacea hay 299 comprobantes con ese valor. Ofrecerlo en el desplegable mete
...
describe('esNoAplica', () => {
  ...
  it('la lista no lo ofrece', () => {
    const docs = [{ proveedor: 'N/A' }, { proveedor: 'Pinito' }, { proveedor: 'N/A' }]
    expect(listaDeProveedores(docs)).toEqual(['Pinito'])
  })
})

// El rubro se DEDUCE de donde aparece el proveedor, porque no hay tabla donde
// guardarlo. Los fixtures son los nombres y las categorias reales de Panacea.
describe('proveedoresPorRubro', () => {
  it('quien tiene un lote es material', () => {
    const r = proveedoresPorRubro({
      lotes: [{ proveedor: 'Coop. Panacea' }],
      documentos: [],
    })
    expect(r).toEqual([{ nombre: 'Coop. Panacea', rubro: 'material' }])
  })

  it('quien tiene comprobantes de Aprovisionamiento es material aunque no tenga lote', () => {
    // Son cuatro casos reales en Panacea: comprados a los que todavia no se les
    // cargo ningun lote. Con la senal del lote sola quedarian sin clasificar.
    const r = proveedoresPorRubro({
      lotes: [],
      documentos: [{ proveedor: 'Luciano Simonit', categoria: 'Aprovisionamiento' }],
    })
  ...
  it('las categorias del formulario tambien son insumos, no material', () => {
    ...
    const r = proveedoresPorRubro({
      lotes: [],
      documentos: [{ proveedor: 'Pinito', categoria: 'Sustrato' }],
    })
  ...
  it('material gana sobre insumos cuando estan las dos senales', () => {
    ...
    const r = proveedoresPorRubro({
      lotes: [{ proveedor: 'Graciela' }],
      documentos: [{ proveedor: 'Graciela', categoria: 'Gasto Operativo' }],
    })
    expect(r[0].rubro).toBe('material')
  })
  ...
  it('la grafia que se ofrece sigue siendo la mas usada', () => {
    ...
    const r = proveedoresPorRubro({
      lotes: [],
      documentos: [
        { proveedor: 'Pinito', categoria: 'Sustrato' },
        { proveedor: 'Pinito', categoria: 'Sustrato' },
        { proveedor: 'PINITO', categoria: 'Sustrato' },
      ],
    })
    expect(r).toEqual([{ nombre: 'Pinito', rubro: 'insumos' }])
  })
