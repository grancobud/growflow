...
describe('avisosDeFichaNueva', () => {
  ...
  const PADRON = [
    { id: 'a', dni: '38120886', telefono: '3624883333', nombre_completo: 'Juan Albez' },
    { id: 'b', dni: null, telefono: '3624265489', nombre_completo: 'Patricio Amarilla' },
    { id: 'c', dni: '30111222', telefono: '3795023447', nombre_completo: 'Natalia Morel' },
  ]

  it('una ficha limpia no dispara ningun aviso', () => {
    expect(avisosDeFichaNueva(
      { dni: '35693266', telefono: '3624111222', nombre_completo: 'Esteban Muruzabal' },
      PADRON)).toEqual([])
  })

  it('avisa cuando el DNI ya esta en el padron', () => {
    const av = avisosDeFichaNueva(
      { dni: '38120886', telefono: '3624999999', nombre_completo: 'Juan Albez' }, PADRON)
    expect(av.some(a => a.includes('ya esta en el padron') || a.includes('ya está en el padrón'))).toBe(true)
    expect(av.some(a => a.includes('Juan Albez'))).toBe(true)
  })

  it('compara el DNI sin los puntos', () => {
    // "38.120.886" y "38120886" son el mismo documento.
    expect(avisosDeFichaNueva(
      { dni: '38.120.886', telefono: null, nombre_completo: 'Juan Albez' }, PADRON).length)
      .toBeGreaterThan(0)
  })

  it('marca un CUIL de once digitos como documento imposible', () => {
    const av = avisosDeFichaNueva(
      { dni: '27000111829', telefono: null, nombre_completo: 'Rocio Gimenez' }, PADRON)
    expect(av.some(a => a.includes('11 d'))).toBe(true)
  })

  it('marca los once ceros, que pasaban por tener largo suficiente', () => {
    expect(avisosDeFichaNueva(
      { dni: '00000000000', telefono: null, nombre_completo: 'Melisa Laguna' }, PADRON).length)
      .toBeGreaterThan(0)
  })

  it('con el mismo telefono Y el mismo nombre dice que es la misma persona', () => {
    const av = avisosDeFichaNueva(
      { dni: '0', telefono: '3624265489', nombre_completo: 'Patricio Amarilla' }, PADRON)
  ...
  it('con el mismo telefono pero otro nombre solo sugiere confirmarlo', () => {
    ...
    const av = avisosDeFichaNueva(
      { dni: '40111222', telefono: '3795023447', nombre_completo: 'Natalia Godoy' }, PADRON)
  ...
  it('editar una ficha no la marca como duplicado de si misma', () => {
    expect(avisosDeFichaNueva(
      { dni: '38120886', telefono: '3624883333', nombre_completo: 'Juan Albez' },
      PADRON, 'a')).toEqual([])
  })
})
...
describe('fichaSospechosa', () => {
  const padron = [
    { id: 'a', nombre_completo: 'Maria Jose Gonzalez', dni: null, telefono: '3624 55-1234', activo: true },
  ...
  it('encuentra a quien esta sin DNI, por el nombre', () => {
    const r = fichaSospechosa({ nombre: 'Maria Jose Gonzalez', dni: '27999888' }, padron)
  ...
  it('aguanta tildes, mayusculas y espacios de mas', () => {
    expect(fichaSospechosa({ nombre: 'maria jose gonzalez', dni: '27999888' }, padron)?.ficha.id).toBe('a')
    expect(fichaSospechosa({ nombre: '  MARIA   JOSE  GONZALEZ ', dni: '27999888' }, padron)?.ficha.id).toBe('a')
  })
...
describe('decidirSolicitud contra una ficha sin DNI', () => {
  const ctx = {
    padron: [{ id: 'a', nombre_completo: 'Maria Jose Gonzalez', dni: null, activo: true }],
    vinculados: 10, topeVinculados: 150, sociosQueSostieneElCultivo: 100,
  }

  it('frena en vez de duplicar, y dice por que', () => {
    const d = decidirSolicitud({ nombre: 'Maria Jose Gonzalez', dni: '27999888' }, ctx)
    expect(d.via).toBe('frenar')
 ...
 * en el tope». Panacea tenía 150 socios activos y el tope en 150 — pero CERO
...
describe('contarVinculados', () => {
  ...
  it('un padrón lleno de socios SIN vincular no bloquea ninguna alta', () => {
    ...
    const d = puedeAprobarseSola(
      { nombre: 'Luis Sebastian Ruvira', dni: '34000222' },
...
describe('esperandoAlta', () => {
  ...
  it('«verificando» NO es un callejón sin salida', () => {
    // El caso de Luis Sebastian Ruvira, 09/09/2026: completó /sumate, alguien
