...
describe('vigenteDe', () => {
  it('encuentra el estatuto vigente', () => {
    const d = vigenteDe([doc({ tipo: 'Estatuto', titulo: 'Coop Panacea' })], 'Estatuto')
    expect(d?.titulo).toBe('Coop Panacea')
  })
  ...
  it('la entidad parada no apaga su estatuto', () => {
    const d = vigenteDe([doc({
      ...
      persona_juridica: 'Cooperativa de Trabajo Panacea Limitada',
    })], 'Estatuto')
    expect(d?.persona_juridica).toBe('Cooperativa de Trabajo Panacea Limitada')
  })
})
