...
const ENTIDAD = { razon_social: 'Asoc. Civil Panacea', cuit: '30-11111111-1' } as Entidad

const pac = (p: Partial<Paciente>): Paciente => ({
  id: 'x', nombre_completo: 'Juan Pablo Duarte', apellido: 'Duarte', nombres: 'Juan Pablo',
...
describe('armarCarnet', () => {
  it('el nombre va con el APELLIDO primero', () => {
    expect(armarCarnet(pac({}), ENTIDAD, HOY).nombre).toBe('Duarte, Juan Pablo')
  })
  ...
  it('dice lo que falta en vez de disimularlo', () => {
    ...
    expect(c.nombre).toBe('Duarte, Juan Pablo')
  })
...
describe('del carnet al mostrador', () => {
  const padron = [
    { id: 'a', codigo: 'PAC-010', nombre_completo: 'Juan Pablo Duarte' },
  ...
  it('y a veces la URL entera de la que el código es el final', () => {
    expect(buscarPorCodigo(padron, 'https://growflow-panacea.pages.dev/s/PAC-010')?.id).toBe('a')
  })
