...
// Son las que se construyeron contrastando la planilla de Panacea hoja por hoja
...
// sigue dando lo que Panacea ya vio en pantalla y dio por bueno.
...
describe('balanceMateria', () => {
  it('suma lo comprado ademas de lo cosechado', () => {
    // El bug real: Panacea no cultiva, compra. Con sólo cosechas el balance daba
...
describe('produccionPorProductor', () => {
  ...
  const lotes: LoteIngreso[] = [
    { gramos_totales: 3146, unidad: 'g', proveedor: 'Coop. Panacea' },
    { gramos_totales: 2, unidad: 'u', proveedor: 'Coop. Panacea' },
    { gramos_totales: 1300, unidad: 'g', proveedor: 'Graciela' },
    { gramos_totales: 50, unidad: 'u', proveedor: 'Adri Varela' },
    { gramos_totales: 94, unidad: 'g', proveedor: null },
  ]

  it('suma solo gramos y cuenta aparte las otras unidades', () => {
    // La planilla da 3.150 para Coop. Panacea porque cuenta los frascos como gramos.
    const coop = produccionPorProductor(lotes).find(x => x.productor === 'Coop. Panacea')!
    expect(coop.gramos).toBe(3146)
    expect(coop.otrasUnidades).toBe(1)
    expect(coop.lotes).toBe(2)
  })

  it('los 50 de Adri Varela son unidades, no gramos', () => {
    // A-PR25726: la planilla los suma como 50 g y por eso le da 242,5.
    const adri = produccionPorProductor(lotes).find(x => x.productor === 'Adri Varela')!
  ...
  it('ordena de mayor a menor', () => {
    expect(produccionPorProductor(lotes)[0].productor).toBe('Coop. Panacea')
  })
})
...
describe('resumenCaja y enRango', () => {
  ...
  // SOLO efectivo y transferencia. En Panacea eso escondia 92 asientos por
  // $17,7M —«Mixto», sin medio, y un «Tranferencia» mal escrito— debajo de un
