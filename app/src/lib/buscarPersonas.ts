 ...
 * el mismo criterio: hay «Juan Pablo Duarte» y hay «Alvarez Walter Ariel».
 *
...
export function filtrarPersonas<T extends PersonaBuscable>(
  ...
  // «PAC-024» queda como una sola palabra y no matchea contra los tokens «pac»
  // y «024» en los que se partió el código.
  const palabras = normalizar(consulta).split(/[\s.-]+/).filter(Boolean)
  if (palabras.length === 0) return personas

  return personas.filter(p => {
    // El código y el DNI entran a la misma bolsa: quien tiene el PAC-117 a mano
 ...
 * renglones idénticos y elegir es adivinar. En el padrón de Panacea pasó de
 * verdad: hubo tres «Rocio Gimenez» al mismo tiempo.
 */
 ...
 *
 * «Duarte, Juan Pablo». Es la convención de toda la app, no de una pantalla:
