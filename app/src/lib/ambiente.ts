...
export interface Sala {
   ...
   * los seis: el target de Panacea es 24-26 C, y con el rango de etapa (22-28)
 ...
 */
/**
 * Cuantos dias de calendario pasaron desde la ultima lectura. 0 = hoy.
 *
 * Existe porque el Panel decia «falta la lectura de la tarde» tanto si faltaba
 * la de hoy como si no se cargaba nada hacia seis dias: el 16/09/2026 la ultima
 * era del 11/09 y el aviso sonaba a un olvido del turno. Un hueco de dias es
 * otra cosa y tiene que leerse como tal.
 *
 * Se cuenta por fecha local y no por horas: una lectura de anoche a las 23 h y
 * hoy a las 8 h estan a 9 horas, pero son dias distintos.
 */
export function diasSinLectura(ultima: Lectura | null, hoy: Date = new Date()): number | null {
  if (!ultima) return null
  const a = new Date(fechaLocal(new Date(ultima.medido_en)) + 'T00:00:00')
  const b = new Date(fechaLocal(hoy) + 'T00:00:00')
  return Math.max(0, Math.round((b.getTime() - a.getTime()) / 86400000))
}

/** El titulo del paso 1 del Panel cuando falta la lectura. */
export function tituloFaltaLectura(diasSin: number | null, turno: string): string {
  if (diasSin === null) return 'Tomar los valores de la sala — todavía no se cargó ninguna'
  if (diasSin >= 2) return `Tomar los valores de la sala — sin lecturas hace ${diasSin} días`
  if (diasSin === 1) return 'Tomar los valores de la sala — sin lecturas desde ayer'
  return `Tomar los valores de la sala — falta la lectura de la ${turno}`
}
