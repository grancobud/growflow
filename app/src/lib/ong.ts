...
export const ongService = {
  ...
  async guardarDispensa(d: Partial<Dispensa>): Promise<{ dispensa: Dispensa; caja: AccionAsiento[] }> {
    ...
    // —que en Panacea son 1.690 filas— solo para poder guardar una entrega.
 ...
 *   alquiler / Alquiler                    18 comprobantes  $5.550.100
 *   viaticos / Viaticos / Viáticos         40 comprobantes    $587.000
 *
 ...
 * de $587.500 sobre $62,9M movidos —un 0,9%— y varios meses dan NEGATIVO. Un
 ...
 *
 * Con los datos de Panacea la diferencia no es un detalle: el alquiler figura
 * cargado en $625.000 —el valor de HOY— y en 13 meses fue de $130.000 a
 * $625.000, casi cinco veces. Aplicar el valor de hoy a todo el historico infla
...
export interface Dispensa {
   ...
   * $9.825.594 «sin discriminar» sobre 1.827 asientos, y el efectivo da
   * −$9.076.000, que es un número imposible.
   */
 ...
 * operación normal: una entrega de 0 gramos que cobró $750.000 no es un dato
 * sucio, es un socio cancelando lo que adeudaba.
 *
 * Lo confirmó Panacea: la plata que entra sin salir gramos es el pago de una
 ...
 *   dicen «Dispensa a PAC-053» vienen de la planilla vieja y ese texto es el
 ...
 * («Pago a Graciela»), para que el pago nuevo caiga en el mismo rubro que los
...
export function asientoDeEntrega(d: Dispensa, asientos: AsientoCaja[]): AccionAsiento[] {
  ...
  for (const linea of quiero) {
    ...
    // «Dispensa a PAC-053» vienen de la planilla vieja, y ese texto es el rastro
...
export function revisarDispensa(
  ...
  // RN-01: no vinculado. Pasa de bloqueo a AVISO por decisión de Panacea del
  ...
  // Al 23/08/2026 Panacea tenía 209 asociados sin mandato, y no por descuido:
...
export interface LoteIngreso {
   ...
   * lotes que lo tenían cargado daban $8.156 por gramo contra $9.871 de aporte,
   * y esos $1.715 de diferencia no llegaban a cubrir los gastos operativos.
   */
 ...
 * a terceros.
 *
 * En Panacea eran 30 lotes por 4.837 g, y la consecuencia visible es que la
 ...
 * compra a terceros quedaba marcada `inconsistente` para siempre: 6.747 g
...
export function balanceMateria(
  ...
  // con 2.719 g de flor y no significaba nada. Son 37 dispensas y 6 lotes.
 ...
 * múltiplos de $1.000, o sea que cobran redondo y la tarifa por gramo es el
 ...
 *
 * El Sistema de Gestión —la planilla con la que Panacea operó todo el primer
 ...
 *
 * Panacea decidió que la planilla es la fuente correcta, así que el sistema se
...
export interface ResumenCaja {
   ...
   *
   * Existe porque el arqueo lo NECESITA. En Panacea son 92 asientos por
...
//
// POR QUE EXISTE (31/08/2026). La caja de efectivo de Panacea daba -$10,2M, que
// es fisicamente imposible: no se pueden sacar mas billetes de los que entraron.
// Medido: hay CINCO asientos de traspaso interno en trece meses, por $692.000.
 ...
 * sea porque es como lo lleva Panacea:
 ...
 * datos de hoy eso da $49.892 contra los $70.154 reales: subestima el aporte un
 ...
 *
 * La planilla de la que salieron los datos de Panacea no tenia el concepto de
 * cultivo propio, asi que un año de produccion propia entro como ordenes de
 * compra a si mismos: 46 ordenes y $26.376.500, el 81% de todo el saldo. La
 * pantalla decia $32.442.000 de deuda cuando con terceros eran unos $6,2M.
 *
 ...
 * `v_saldo_proveedores`— y no por parecido. «Contiene Panacea» agarraria tambien
 ...
 */
export function diasHasta(fecha: string, hoy: string = hoyLocal()): number {
...
export function chequeosCoherencia(datos: {
  ...
  if (nombres.length > 0) {
    ...
    // "Diego Ramirez" en los pagos, y este cruce no los veía: comparaba
    ...
    // "Emiilio", "Facu Ayala" por "Facundo Suarez". Para eso hace falta la lista
  ...
  // porque las dos lecturas dan numeros distintos y solo Panacea sabe cual paso.
  //
  ...
  // «$5.844.880 aportados», «$6.160.320 que la asociación absorbió». Al director
  ...
  //
  // Con FECHA DE CORTE desde el 30/08/2026. Antes, los 30 lotes por 4.837 g de
  // Panacea daban una alerta que no se podía apagar nunca: la cosecha de un
  ...
  // ve nada de lo que no tiene fecha. En Panacea, al 27/08/2026, las 65 fichas
  ...
  //
  // "En tramite" NO cuenta como problema: es la decision de Panacea —quien tiene
  ...
  // designarlos». Estaba al revés, y lo corrigió Panacea. El circuito público ya
  ...
  // 0, por $15.558.110, a 25 pacientes y sobre 34 lotes. Una tiene $1.000.000 de
  // aporte y cero gramos.
  //
  // Al aporte por gramo normal eso son ~1.345 g que salieron y no figuran: un
  ...
  if (ds23b.length > 0) {
    ...
    // decide Panacea; los vacíos ya se revisaron y se dejaron así a propósito.
  ...
  // qué medio entraron.
  //
  // En Panacea esto apareció el 28/08/2026 y es el caso que le da nombre al
  // cruce. Las trece facturas de luz —$700.000 por mes durante un año— estaban
  // cargadas sin medio de pago. Puestas en efectivo, que es como se pagan, la
  // caja pasó a −$9.076.000: no porque el dato nuevo estuviera mal, sino porque
  ...
  if (cajaCoh.length > 0) {
    ...
    c.push(arq.netoEfectivo >= 0
      ...
      : { clave: 'caja_negativa', titulo: 'Caja en efectivo', estado: 'error',
          ...
          // base el 31/08/2026: los aportes de las entregas suman $63.518.539 y
          // la caja tiene EXACTAMENTE $63.518.539 en asientos ligados a una
          ...
          //    interno por $692.000 contra $10,2 millones de efectivo negativo.
          //
  ...
  //
  // Medido en Panacea el 30/08/2026: 158 ingresos por $8.936.000 con el concepto
  ...
  //
  // Ejemplo verificado: PAC-187 del 03/08 tiene una entrega de $90.000 con su
  // asiento de $90.000 «Mixto», y encima dos sueltos de $75.000 y $15.000. La
  // caja cuenta $180.000 por una entrega de $90.000.
  //
  ...
  // El 30/08/2026 marcaba 33 asientos por $2.079.000 cuando lo que de verdad
  // quedaba suelto eran 12 por $714.000.
  //
  ...
  // apertura. El 01/09/2026 este cruce marcó un pago de $40.000 del 29/08 —el
  // día del corte— y agregarle su asiento, que es lo que el cruce pedía, habría
  // dejado la caja en $150.000 contra los $190.000 que declara el Consolidado.
